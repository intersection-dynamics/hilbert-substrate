#!/usr/bin/env python3
# filename: hsf_mesoscape_entanglement_graded_ecology.py
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "HSF mesoscale entanglement graded ecology sandbox. "
            "Birth is sourced by parent-pair entanglement. Pruning is handled through graded support "
            "via a two-channel sustain test: sustaining entanglement and organizer sustain are both tracked, "
            "and lower_support is only strongly favored when both are weak."
        )
    )
    ...
    p.add_argument("--sigma-step", type=float, default=0.20)
    p.add_argument("--edge-step", type=float, default=0.20)

    p.add_argument("--max-parent-pairs", type=int, default=8)
    p.add_argument("--max-children-per-pair", type=int, default=3)
    p.add_argument("--ent-threshold", type=float, default=0.01)
    p.add_argument("--corr-scale", type=float, default=4.0)
    p.add_argument("--corr-weight", type=float, default=0.25)

    p.add_argument("--birth-finalists", type=int, default=4)
    p.add_argument("--lower-finalists", type=int, default=3)
    p.add_argument("--edge-up-finalists", type=int, default=3)
    p.add_argument("--edge-down-finalists", type=int, default=3)

    p.add_argument("--json-out", type=str, default="hsf_mesoscape_entanglement_graded_ecology.json")
    return p.parse_args()
...
def entanglement_sources(
    ...
    for idx, a in enumerate(nodes):
        for b in nodes[idx + 1:]:
            ...
            ent = float(np.tanh(max(0.0, mi)) + float(args.corr_weight) * np.tanh(float(args.corr_scale) * max(0.0, corr)))
...
def generate_birth_candidates(
    ...
    for row in parent_rows[: int(args.max_parent_pairs)]:
        ...
        for child in range(len(sigma)):
            ...
            vacancy = float(1.0 - np.clip(sigma[child], 0.0, 1.0))
            novelty = float(1.0 - 0.5 * np.clip(attach, 0.0, 1.0))
            priority = vacancy * novelty
            children.append((priority, child))
        children.sort(reverse=True)
        for _, child in children[: int(args.max_children_per_pair)]:
            out.append({
                "parents": [a, b],
                "child": int(child),
                "ent_source": float(row["source"]),
                "ent_mi": float(row["mi"]),
                "ent_corr": float(row["corr"]),
                "ent": float(row["ent"]),
            })
    return out


def batched_birth_proxy(candidates: List[Dict[str, Any]], sigma: np.ndarray, interface_commitment: Dict[Edge, float]):
    if not candidates:
        return []
    W = np.zeros((len(sigma), len(sigma)), dtype=np.float64)
    for (i, j), w in interface_commitment.items():
        W[i, j] = float(w)
        W[j, i] = float(w)
    out: List[Dict[str, Any]] = []
    for cand in candidates:
        a, b = int(cand["parents"][0]), int(cand["parents"][1])
        child = int(cand["child"])
        attach = float(W[a, child] + W[b, child])
        vacancy = float(1.0 - np.clip(sigma[child], 0.0, 1.0))
        novelty = float(1.0 - 0.5 * np.clip(attach, 0.0, 1.0))
        row = dict(cand)
        row["birth_proxy"] = float(cand["ent_source"]) * vacancy * novelty
        out.append(row)
    out.sort(key=lambda r: r["birth_proxy"], reverse=True)
    return out


def organizer_sustain_per_node(
    sigma: np.ndarray,
    interface_commitment: Dict[Edge, float],
    core_before: Optional[Dict[str, Any]],
) -> Dict[int, float]:
    sustain = {i: 0.0 for i in range(len(sigma))}
    if core_before is None or core_before.get("core_pair") is None:
        return sustain
    cp = core_before["core_pair"]
    if not isinstance(cp, (list, tuple)) or len(cp) != 2:
        return sustain
    a, b = int(cp[0]), int(cp[1])
    core_score = float(np.tanh(max(0.0, float(core_before.get("score", 0.0)))))
    for i in range(len(sigma)):
        if i == a or i == b:
            sustain[i] = core_score
        else:
            attach = max(
                float(interface_commitment.get(canonical_edge(i, a), 0.0)),
                float(interface_commitment.get(canonical_edge(i, b), 0.0)),
            )
            sustain[i] = float(core_score * attach)
    return sustain


def lower_support_proxies(
    sigma: np.ndarray,
    active_nodes: Set[int],
    parent_rows: List[Dict[str, Any]],
    interface_commitment: Dict[Edge, float],
    ...
    sustain_ent = {i: 0.0 for i in active_nodes}
    for row in parent_rows:
        a, b = int(row["parents"][0]), int(row["parents"][1])
        src = float(row["source"])
        if a in sustain_ent:
            sustain_ent[a] = max(sustain_ent[a], src)
        if b in sustain_ent:
            sustain_ent[b] = max(sustain_ent[b], src)
    sustain_org = organizer_sustain_per_node(sigma, interface_commitment, core_before)

    rows: List[Tuple[float, int, Dict[str, float]]] = []
    for i in active_nodes:
        ent_keep = float(np.clip(sustain_ent.get(i, 0.0), 0.0, 1.0))
        org_keep = float(np.clip(sustain_org.get(i, 0.0), 0.0, 1.0))
        weak_ent = float(1.0 - ent_keep)
        weak_org = float(1.0 - org_keep)
        proxy = float(np.clip(sigma[i], 0.0, 1.0) * weak_ent * weak_org)
        rows.append((proxy, int(i), {
            "sustain_entanglement": ent_keep,
            "sustain_organizer": org_keep,
            "weak_entanglement": weak_ent,
            "weak_organizer": weak_org,
        }))
    rows.sort(reverse=True, key=lambda x: x[0])
    return rows


def edge_up_proxies(interface_commitment: Dict[Edge, float], birth_rows: List[Dict[str, Any]]) -> List[Tuple[float, Edge]]:
    rows: List[Tuple[float, Edge]] = []
    seen: Set[Edge] = set()
    for row in birth_rows:
        a, b = int(row["parents"][0]), int(row["parents"][1])
        child = int(row["child"])
        src = float(row["ent_source"])
        for e in (canonical_edge(a, child), canonical_edge(b, child)):
            if e in seen:
                continue
            seen.add(e)
            gap = float(1.0 - np.clip(interface_commitment.get(e, 0.0), 0.0, 1.0))
            rows.append((src * gap, e))
    rows.sort(reverse=True, key=lambda x: x[0])
    return rows


def edge_down_proxies(
    interface_commitment: Dict[Edge, float],
    ...
    for row in parent_rows[:4]:
        a, b = int(row["parents"][0]), int(row["parents"][1])
        protected.add(canonical_edge(a, b))
    if core_before is not None and core_before.get("core_pair") is not None:
        cp = core_before["core_pair"]
        if isinstance(cp, (list, tuple)) and len(cp) == 2:
            protected.add(canonical_edge(int(cp[0]), int(cp[1])))

    rows: List[Tuple[float, Edge]] = []
    for e in active_edges:
        w = float(interface_commitment.get(e, 0.0))
        protection = 0.0 if e in protected else 1.0
        rows.append((protection * w, e))
    rows.sort(reverse=True, key=lambda x: x[0])
    return rows
...
def compact(diag: Dict[str, Any]) -> Dict[str, Any]:
    keep = [
        ...
        "ent_source", "ent_mi", "ent_corr", "ent", "birth_proxy",
        "lower_proxy", "edge_up_proxy", "edge_down_proxy",
        "sustain_entanglement", "sustain_organizer", "weak_entanglement", "weak_organizer",
    ]
...
def run_sim(pcfg: PhysicsConfig, scfg: bk.ScoreConfig, args: argparse.Namespace):
    ...
    for step in range(1, args.total_steps + 1):
        ...
        n_sites = int(baseline_state[4].shape[0])
        core_before = bk.dominant_core_snapshot(
            psi_b, active_nodes_b, active_edges_b, baseline_state[5], GM_MATRICES, xp, n_sites, scfg, link_regs_b
        )

        parent_rows = entanglement_sources(psi_b, sigma, active_nodes_b, xp, args)
        raw_births = generate_birth_candidates(sigma, interface_commitment, parent_rows, args)
        ranked_births = batched_birth_proxy(raw_births, sigma, interface_commitment)
        birth_rows = ranked_births[: int(args.birth_finalists)]

        lower_rows = lower_support_proxies(sigma, active_nodes_b, parent_rows, interface_commitment, core_before)[: int(args.lower_finalists)]
        edge_up_rows = edge_up_proxies(interface_commitment, birth_rows)[: int(args.edge_up_finalists)]
        edge_down_rows = edge_down_proxies(interface_commitment, active_edges_b, parent_rows, core_before)[: int(args.edge_down_finalists)]
        ...
        for row in birth_rows:
            parents = canonical_edge(int(row["parents"][0]), int(row["parents"][1]))
            child = int(row["child"])
            graded_after, meta = prepare_birth_move(graded_before, parents, child, args)
            meta.update({k: row[k] for k in ("ent_source", "ent_mi", "ent_corr", "ent", "birth_proxy")})
        ...
        for proxy, e in edge_down_rows:
            graded_after, meta = prepare_edge_down_move(graded_before, e, args)
            meta["edge_down_proxy"] = float(proxy)
            diag, state_after = score_candidate(meta, graded_before, graded_after, baseline_state, pcfg, scfg, args, xp)
            diags.append(diag)
            if float(diag["deltaF"]) > best_delta:
                best_delta = float(diag["deltaF"])
        ...
        vis_edges = [
            list(e) for e, w in sorted(interface_commitment.items())
            if float(w) > float(args.edge_on_threshold) and e[0] in vis_nodes and e[1] in vis_nodes
        ]
        vis_parent_rows = entanglement_sources(psi, sigma, set(vis_nodes), xp, args) if vis_nodes else []
        ...
        snapshots.append({
            ...
            "top_entangled_pairs": vis_parent_rows[:8],
            "raw_birth_candidate_count": int(len(raw_births)),
            "birth_finalist_count": int(len(birth_rows)),
            "lower_finalist_count": int(len(lower_rows)),
            "edge_up_finalist_count": int(len(edge_up_rows)),
            "edge_down_finalist_count": int(len(edge_down_rows)),
        ...
        if int(args.progress_every) > 0 and step % int(args.progress_every) == 0:
            print(
                ...
                f"deltaF={(accepted['deltaF'] if accepted else None)} "
                f"birth_raw={len(raw_births)} birth_fin={len(birth_rows)} "
                f"lower_fin={len(lower_rows)} up_fin={len(edge_up_rows)} down_fin={len(edge_down_rows)} cand={len(diags)}"
            )

    return {
        "script": "hsf_mesoscape_entanglement_graded_ecology.py",
        ...
        "run_config": {
            ...
            "birth_finalists": int(args.birth_finalists),
            "lower_finalists": int(args.lower_finalists),
            "edge_up_finalists": int(args.edge_up_finalists),
            "edge_down_finalists": int(args.edge_down_finalists),
            "ent_threshold": float(args.ent_threshold),
            "principle": "preserve the full ecology while evaluating it faster through cached baseline evolution, cached pair entanglement, cheap per-class proxies, exact scoring only on reduced finalists, and two-channel graded-support pruning diagnostics",
        },
