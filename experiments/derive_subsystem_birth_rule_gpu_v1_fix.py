...
Total redesign:
- GPU-first (CuPy) implementation, with NumPy fallback for testing.
- No scipy sparse matrices.
- No expm_multiply.
- Hamiltonian action is applied directly to the many-body state tensor.
- Time evolution uses RK4 on the Schrödinger equation.
- Birth/death classifier is the mixed-regime v4-style logic, but built on the new core.

Intended use:
    python derive_subsystem_birth_rule_gpu_v1.py --device gpu --json-out out.json

Notes:
- This is designed for qutrit subsystems, N <= 9 practical.
- The numerical hot path is the Hamiltonian application, now GPU-friendly.
- Reduced density matrices and entropies are computed from the full state when needed.
"""
import argparse
import json
