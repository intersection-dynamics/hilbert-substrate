...
class SimConfig:
    ...
    reabsorb_every: int
    reabsorb_lookahead_windows: int
    basis0_fidelity_floor: float
    organizer_fidelity_floor: float
    neighbor_mi_retention_floor: float
    cooldown_evals: int
    max_reabsorptions_per_eval: int
    device: str
    progress_every: int
    json_out: str
...
def spawn_births(chosen, active_nodes, dormant_nodes, active_edges, edge_strengths, local_coeffs, spawn_pair_scale, rng, eval_index):
    ...
    for row, new_node in chosen:
        ...
        birth_eval[new_node] = eval_index
        events.append({"parents": [i, j], "new_node": new_node})
    return events, birth_eval
...
def neighbor_pair_mi_sum(psi, neighbors, n_sites, xp):
    if len(neighbors) < 2:
        return 0.0
    tot = 0.0
    count = 0
    for a, b in combinations(sorted(neighbors), 2):
        tot += mutual_information_from_state(psi, a, b, n_sites, xp)
        count += 1
    return float(tot / max(1, count))
...
def attempt_no_forgetting_reabsorption(
    psi,
    active_nodes,
    active_edges,
    edge_strengths,
    ...
    for n in active_nodes:
        ...
        fid0_now = basis0_fidelity(rho_n)
        degree = len(adj.get(n, []))
        candidates.append((-(fid0_now), degree, n))  # highest basis0 fidelity first, then low degree
    candidates.sort()

    accepted = []
    for _, _, n in candidates[: max(1, 4 * cfg.max_reabsorptions_per_eval)]:
        if len(accepted) >= cfg.max_reabsorptions_per_eval:
            break
        ...
        psi_ctrl = evolve_windows(
            psi.copy(),
            cfg.reabsorb_lookahead_windows,
            cfg,
            active_nodes,
            active_edges,
            local_coeffs,
        ...
        mi_neighbors_ctrl = neighbor_pair_mi_sum(psi_ctrl, neighbors, cfg.n_max, xp)
        mi_neighbors_test = neighbor_pair_mi_sum(psi_test, neighbors, cfg.n_max, xp)
        ...
        passed = (
            fid0_test >= cfg.basis0_fidelity_floor
            and org_fid >= cfg.organizer_fidelity_floor
            and neighbor_retention >= cfg.neighbor_mi_retention_floor
        )

        evt = {
            "candidate_node": int(n),
        ...
        if passed:
            if n in active_nodes:
                active_nodes.remove(n)
            active_edges[:] = [e for e in active_edges if n not in e]
            for e in list(edge_strengths.keys()):
                if n in e:
                    edge_strengths.pop(e, None)
            local_coeffs[n] = np.zeros(8, dtype=float)
            accepted.append(evt)
...
def run_sim(cfg: SimConfig):
    ...
    while step < cfg.total_steps:
        ...
        reabs_now = []
        if total_evals % cfg.reabsorb_every == 0:
            reabs_now = attempt_no_forgetting_reabsorption(
                psi=psi,
            ...
            for evt in reabs_now:
                evt["step"] = step
            reabsorption_events.extend(reabs_now)

        core_after = dominant_core_snapshot(psi, active_nodes, active_edges, edge_strengths, GM, xp, cfg.n_max)
        metric = metric_snapshot(active_nodes, active_edges, edge_strengths)
        curr_core = core_after["core_pair"] if core_after is not None else None
        ...
        snapshots.append({
            ...
            "reabsorbed_this_window": int(sum(1 for e in reabs_now if e["accepted"])),
        ...
        if cfg.progress_every > 0 and (total_evals % cfg.progress_every == 0):
            print(
                ...
                f"edges={len(active_edges):3d} core={curr_core} births={len(labeled)} "
                f"reabsorbed={sum(1 for e in reabs_now if e['accepted'])}"
            )
    ...
    summary = {
        ...
        "n_reabsorption_attempts": int(len(reabsorption_events)),
        "n_reabsorptions_accepted": int(sum(1 for e in reabsorption_events if e["accepted"])),
    ...
    return {
        ...
        "reabsorption_events": reabsorption_events,
        "snapshots": snapshots,
        "summary": summary,
...
def parse_args():
    ap = argparse.ArgumentParser(description="HSF mesoscape with no-forgetting support demotion / reabsorption.")
    ...
    ap.add_argument("--reabsorb-every", type=int, default=1)
    ap.add_argument("--reabsorb-lookahead-windows", type=int, default=2)
    ap.add_argument("--basis0-fidelity-floor", type=float, default=0.995)
    ap.add_argument("--organizer-fidelity-floor", type=float, default=0.999)
    ap.add_argument("--neighbor-mi-retention-floor", type=float, default=0.95)
    ap.add_argument("--cooldown-evals", type=int, default=2)
    ap.add_argument("--max-reabsorptions-per-eval", type=int, default=2)

    ap.add_argument("--progress-every", type=int, default=1)
    ap.add_argument("--json-out", type=str, default="gpu_mesoscape_no_forgetting_reabsorption.json")
...
def main():
    ...
    cfg = SimConfig(
        ...
        reabsorb_every=args.reabsorb_every,
        reabsorb_lookahead_windows=args.reabsorb_lookahead_windows,
        basis0_fidelity_floor=args.basis0_fidelity_floor,
        organizer_fidelity_floor=args.organizer_fidelity_floor,
        neighbor_mi_retention_floor=args.neighbor_mi_retention_floor,
        cooldown_evals=args.cooldown_evals,
        max_reabsorptions_per_eval=args.max_reabsorptions_per_eval,
        device=args.device,
