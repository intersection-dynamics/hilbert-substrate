...
# -*- coding: utf-8 -*-
"""
gauss_commutator_su3_v4_unitary_vs_logm.py
=========================================

Purpose
-------
You asked for the decisive follow-up after v3:

  1) Test gauge commutation directly on the *unitary superoperator*:
       U_super = U_eff ⊗ U_eff^*
     via ||[G, U_super]||_F

  2) Compare to the logm-derived generator test (v3 style):
       H_super = H_eff ⊗ I - I ⊗ H_eff^T
     via ||[G, H_super]||_F

This tells us whether Gauss breaking is:
  - mainly an artifact of logm branch / Heff extraction, or
  - already present at the unitary level.

We use the *doubled link* representation so L/R commute exactly:
  link Hilbert: C^3 ⊗ C^3 (dim 9)
  L^a = T^a ⊗ I
  R^a = I ⊗ (T^a)^T

We keep the microscopic model same as v1/v3:
  sites (d_site=2 or 3) + fundamental bonds (d_B=3), on a single plaquette.

Workflow
--------
For each (coupling in {aligned,mixed}, Delta in list):
  - build H_full
  - compute U_eff on bond manifold (dim 3^4=81):
      U_eff = P exp(-i t H_full) P
      polar reunitarize -> W  (unitary approx)
  - define:
      U_super = W ⊗ W^*   (dim 81^2 = 6561)
      H_eff   = (i/t) logm(W) (Hermitianized)
      H_super = H_eff ⊗ I - I ⊗ H_eff^T
  - build Gauss generators G_x^a on doubled links (dim 6561):
      G_x^a = L_out(link_out,a) - R_in(link_in,a)
  - report normalized commutator metrics for BOTH:
      epsU = ||[G, U_super]||_F / (||G||_F ||U_super||_F)
      epsH = ||[G, H_super]||_F / (||G||_F ||H_super||_F)

Optional
--------
--npz <path> : use basis_both_aligned/mixed from your echo NPZ for T^a
               instead of canonical Gell-Mann.

Run (Windows one-liners)
------------------------
python gauss_commutator_su3_v4_unitary_vs_logm.py --Delta 6,8,10,14 --g 0.30 --t 0.10 --seed 0 --coupling both
python gauss_commutator_su3_v4_unitary_vs_logm.py --Delta 6,8,10,14 --g 0.30 --t 0.10 --seed 0 --coupling both --npz hsf_out\\echo_algebra_step1_qutrit_su3_LR_bases_20260219_144601.npz

Outputs
-------
./hsf_out/gauss_commutator_su3_v4_<timestamp>.json
...
def build_edge_couplings(d_site: int, d_bond: int, coupling: str, rng: np.random.Generator):
    if d_bond != 3:
        raise ValueError("v4 assumes d_bond=3 (SU(3) bonds).")
...
# -------------------------
# Bond-only effective unitary W and Heff
# -------------------------
...
# -------------------------
# True link-end Gauss generators on doubled link Hilbert (dim 6561)
# -------------------------
...
# -------------------------
# Metrics: [G, U_super] and [G, H_super]
# -------------------------
...
def run_one_case(spec: CaseSpec, T_basis_3x3) -> dict:
    ...
    Heff, diag_heff = compute_Heff_from_W(spec, W)

    # Doubled-space targets
    U_super = lift_U_super_sparse(W)
    H_super = lift_H_super_sparse(Heff)

    # Gauss generators (true commuting L/R)
    G_ops = build_gauss_ops_doubled(T_basis_3x3)

    # Metrics
    metrics_U = comm_metrics_for_target(U_super, G_ops)
    metrics_H = comm_metrics_for_target(H_super, G_ops)
...
def main():
    ...
    results = {
        "meta": {
            ...
            "note": "v4 compares Gauss commutators on U_super vs H_super (logm).",
        },
        "runs": [],
    }
    ...
    for coupling in coupling_modes:
        ...
        for Delta in Deltas:
            ...
            runrec = {
                ...
                "ratio_eps_mean_U_qutrit_over_qubit": (
                    (r3["U_metrics"]["eps_mean"] / r2["U_metrics"]["eps_mean"])
                    if r2["U_metrics"].get("eps_mean", 0) > 0 else None
                ),
                "ratio_eps_max_U_qutrit_over_qubit": (
                    (r3["U_metrics"]["eps_max"] / r2["U_metrics"]["eps_max"])
                    if r2["U_metrics"].get("eps_max", 0) > 0 else None
                ),
                "ratio_eps_mean_H_qutrit_over_qubit": (
                    (r3["H_metrics"]["eps_mean"] / r2["H_metrics"]["eps_mean"])
                    if r2["H_metrics"].get("eps_mean", 0) > 0 else None
                ),
                "ratio_eps_max_H_qutrit_over_qubit": (
                    (r3["H_metrics"]["eps_max"] / r2["H_metrics"]["eps_max"])
                    if r2["H_metrics"].get("eps_max", 0) > 0 else None
                ),
            }
            ...
            # Console summary
            print("------------------------------------------------------------")
            print(f"[v4] Δ={Delta:.3f} coupling={coupling} g={args.g:.3f} t={args.t:.3f}  generators={gen_src}")

            print("  U_super commutators:")
            print(f"    qubit  eps_mean={r2['U_metrics'].get('eps_mean', None)}  eps_max={r2['U_metrics'].get('eps_max', None)}")
            print(f"    qutrit eps_mean={r3['U_metrics'].get('eps_mean', None)}  eps_max={r3['U_metrics'].get('eps_max', None)}")
            print(f"    ratio  mean={runrec['ratio_eps_mean_U_qutrit_over_qubit']}  max={runrec['ratio_eps_max_U_qutrit_over_qubit']}")

            print("  H_super commutators (logm):")
            print(f"    qubit  eps_mean={r2['H_metrics'].get('eps_mean', None)}  eps_max={r2['H_metrics'].get('eps_max', None)}")
            print(f"    qutrit eps_mean={r3['H_metrics'].get('eps_mean', None)}  eps_max={r3['H_metrics'].get('eps_max', None)}")
            print(f"    ratio  mean={runrec['ratio_eps_mean_H_qutrit_over_qubit']}  max={runrec['ratio_eps_max_H_qutrit_over_qubit']}")

    out_dir = ensure_out_dir()
    out_path = os.path.join(out_dir, f"gauss_commutator_su3_v4_{results['meta']['timestamp']}.json")
