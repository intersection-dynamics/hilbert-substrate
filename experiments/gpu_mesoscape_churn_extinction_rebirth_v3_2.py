...
def choose_candidate_births(rows, dormant_nodes, cfg):
    ...
    bootstrap_mode = bool(rows and rows[0].get("bootstrap_mode", False))
    if bootstrap_mode:
        considered = [
            r for r in rows[: max(1, cfg.max_birth_candidates)]
            if r["mi"] >= cfg.bootstrap_mi_floor
            and r["corr"] >= cfg.bootstrap_corr_floor
            and r["score"] >= cfg.bootstrap_birth_score_floor
        ]
        if not considered:
            return []
    else:
    ...
    max_births = 1 if bootstrap_mode else min(cfg.max_births_per_eval, len(dormant_nodes))

    for row in considered:
        if len(chosen) >= max_births:
            break
        i, j = row["pair"]
        if i in used_nodes or j in used_nodes:
            continue
        common = set(row["common_nbrs"])
        if (not bootstrap_mode) and (common & used_common):
            continue
        center = float(row["pair_center"])
        if (not bootstrap_mode) and any(abs(center - c) < cfg.birth_exclusion_radius for c in used_centers):
            continue
        try:
