# hsf_metastable_branch_toy_v4.py
# ------------------------------------------------------------
# HSF-inspired toy model: bias selection into a metastable branch (v4)
#
# v4 implements the two "HSF-clean" mechanisms:
#   (1) SIGNED DRIVE in the tuned region:
#       drive_term ~ drive_gain * drive / (1 + local_mem)
#       (negative drive increases barrier; positive drive decreases barrier)
#
#   (2) BOUNDARY SHELL on edges that cross tuned<->outside:
#       - Boundary edges get a bandwidth bottleneck (harder to export bookkeeping)
#       - Boundary edges get stronger memory persistence/coupling (stickier ledger)
#
# Also keeps v3 diagnostics:
#   - cumulative + rolling branch fractions (trusted view)
#   - hide ticks where rolling captures are too small
#   - prints final delta, SE, z-score
#   - phase-binned pM(phi) plots
#
# NEW in v4:
#   - Optional internal baseline comparison (same seed, same high-level settings):
#       baseline = unsigned drive (max(0,drive)) + NO boundary shell
#       v4       = signed drive + boundary shell
#     Prints how much Δ increases (and z changes) vs baseline.
#
# SAFE MODEL (generic branching, non-nuclear).
#
# Typical run (Windows one-liner):
#   python hsf_metastable_branch_toy_v4.py --N 250 --p_edge 0.03 --T 20000 --lambda_capture 0.09 --drive_frac 0.20 --drive_amp 0.35 --drive_w 0.03 --drive_gain 2.2 --phase_bins 24 --seed 0 --run_name v4_test
# ------------------------------------------------------------
...
class Params:
    ...
    # edge multipliers (inside region and boundary shell)
    region_bandwidth_mult: float
    region_mem_mult: float
    boundary_bandwidth_mult: float   # < 1 makes a bottleneck
    boundary_mem_mult: float         # > 1 makes boundary memory stickier
...
def branch_prob_metastable(barrier_M: float, drive_term: float, base_bias: float) -> float:
    # Higher barrier => lower p(M). drive_term reduces effective barrier if positive;
    # increases effective barrier if negative (signed drive).
...
def plot_outputs(run_dir: str, data: dict, series: dict, ph: dict, p: Params, prefix: str) -> None:
    ...
    # clean branch plot: cumulative + rolling
...
def run_sim(
    ...
    *,
    ...
    One simulation run.
    signed_drive:
      - if True: drive_term uses signed drive (positive helps, negative hurts)
      - if False: drive_term uses max(0, drive) (only helps)
    boundary_shell:
      - if True: boundary edges get special multipliers
      - if False: boundary edges behave like outside edges
    """
    rng = np.random.default_rng(rng_seed)

    # graph + region come from this RNG so baseline and v4 are comparable by seed
    ...
    # precompute edge classes
    if E > 0:
        ru = region[u].astype(np.int8)
        rv = region[v].astype(np.int8)
        edge_inside = (ru == 1) & (rv == 1)      # both endpoints tuned
        edge_boundary = (ru ^ rv) == 1           # exactly one endpoint tuned
    else:
    ...
    for t in range(T):
        phi = float((p.drive_w * t + p.drive_phase) % TAU)
        ...
        if E > 0:
            ...
            for ei in range(E):
                ...
                boundary = bool(edge_boundary[ei]) if boundary_shell else False
                ...
                # give the gauge register a small drive "twist" inside tuned region edges
                if inside:
                    dth += (drive * 0.15)
        ...
        for i in range(p.N):
            ...
            if in_reg:
                if signed_drive:
                    # signed: positive drive reduces barrier, negative increases barrier
                    drive_term = p.drive_gain * drive / (1.0 + local_mem)
                else:
                    # baseline: only positive helps
...
def main():
    ...
    ap.add_argument("--boundary_bandwidth_mult", type=float, default=0.70, help="boundary bottleneck (<1)")
    ap.add_argument("--boundary_mem_mult", type=float, default=1.35, help="boundary stickiness (>1)")
    ...
    # baseline compare
    ap.add_argument("--compare_baseline", action="store_true", help="also run baseline (unsigned drive, no boundary shell) and report Δ increase")

    # output
    ap.add_argument("--out_dir", type=str, default="hsf_out")
    ap.add_argument("--run_name", type=str, default="metastable_toy_v4")
    ...
    # v4 run
    data_v4 = run_sim(p, p.seed, signed_drive=True, boundary_shell=True)
    series_v4 = compute_series(data_v4, p.rolling_window)
    ph_v4 = phase_stats(data_v4)

    delta_v4, se_v4, z_v4 = print_stats_block("FINAL (CUMULATIVE) BRANCH STATS — v4 (signed drive + boundary shell)", data_v4)

    # save v4 artifacts
    np.savez_compressed(os.path.join(run_dir, "results_v4.npz"), **data_v4, **series_v4, **ph_v4)
    write_csv(os.path.join(run_dir, "log_v4.csv"), data_v4, series_v4)
    write_phase_csv(os.path.join(run_dir, "phase_bins_v4.csv"), ph_v4, p.min_captures_phase)
    plot_outputs(run_dir, data_v4, series_v4, ph_v4, p, prefix="v4")

    # optional baseline comparison
    if p.compare_baseline:
        data_base = run_sim(p, p.seed, signed_drive=False, boundary_shell=False)
        series_base = compute_series(data_base, p.rolling_window)
        ph_base = phase_stats(data_base)

        delta_b, se_b, z_b = print_stats_block("FINAL (CUMULATIVE) BRANCH STATS — baseline (unsigned drive, NO boundary shell)", data_base)

        d_delta = delta_v4 - delta_b
        print("============================================================")
        print("Δ IMPROVEMENT REPORT (v4 - baseline)")
        print("------------------------------------------------------------")
        print(f"delta_v4      = {delta_v4:.6f}")
        print(f"delta_baseline= {delta_b:.6f}")
        print(f"Δ increase    = {d_delta:+.6f}")
        print(f"z_v4          = {z_v4:.3f}")
        print(f"z_baseline    = {z_b:.3f}")
        print(f"Δz            = {z_v4 - z_b:+.3f}")
        print("============================================================")

        np.savez_compressed(os.path.join(run_dir, "results_baseline.npz"), **data_base, **series_base, **ph_base)
        write_csv(os.path.join(run_dir, "log_baseline.csv"), data_base, series_base)
        write_phase_csv(os.path.join(run_dir, "phase_bins_baseline.csv"), ph_base, p.min_captures_phase)
        plot_outputs(run_dir, data_base, series_base, ph_base, p, prefix="baseline")

    print("Run output:", run_dir)
    print("v4 clean plot:", os.path.join(run_dir, "plots", "v4_branch_clean.png"))
    print("v4 phase plot:", os.path.join(run_dir, "plots", "v4_phase_response.png"))
    if p.compare_baseline:
        print("baseline clean plot:", os.path.join(run_dir, "plots", "baseline_branch_clean.png"))
        print("baseline phase plot:", os.path.join(run_dir, "plots", "baseline_phase_response.png"))
