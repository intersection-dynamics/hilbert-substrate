...
def classify_by_spatial(cost: float, spatial_ref: float) -> str:
    """
    Classify outcomes relative to the *spatial reference* cost.

    IMPORTANT:
      - `spatial_ref` is a reference Hamiltonian's cost (e.g., nearest-neighbor on a chosen topology),
        NOT a proven lower bound or global optimum.
      - So we classify "near spatial" by *closeness* to that reference, not by being <= it.

    Heuristic:
      - Near spatial: |cost - spatial_ref| / spatial_ref <= 0.25
      - Glassy / stuck high: cost >= 2 * spatial_ref
      - Otherwise: Quantum fluid / reorganizing
    """
    if spatial_ref <= 0:
        return "Unknown"
    rel = abs(cost - spatial_ref) / spatial_ref
    if rel <= 0.25:
        return "Emergent Geometry (near spatial)"
    if cost >= 2.0 * spatial_ref:
...
def main() -> int:
    ...
    for N in [3, 4, 5]:
        ...
        results["experiments"]["exp1"][str(N)] = {
            ...
            "mean_rel_diff_spatial": float(abs(mean_rec - c_spat) / c_spat) if c_spat > 0 else None,
            "mean_rel_diff_spatial": float(abs(mean_rec - c_spat) / c_spat) if c_spat > 0 else None,
            "regime": regime,
            "trials": trials_out,
        }
    ...
    for name, edges in topologies.items():
        ...
        trapped = "YES" if (abs(final_cost - c_spat) / c_spat) <= 0.25 else "NO"
        ...
        results["experiments"]["exp3"][name] = {
            ...
            "final_rel_diff_spatial": float(abs(final_cost - c_spat) / c_spat),
