...
def find_nullspace_sparse(Cv_sparse, n, mem_budget_gb=20.0):
    """
    Find null space of sparse PSD matrix using polynomial filter + QR.

    Strategy (avoids ARPACK degeneracy problems):
      1. Build B = I - Cv/lambda_max. Null-space eigenvalue=1.0, gap eigenvalue≈0.625
      2. Apply B^p to random probes Y. After p iterations:
         - Null-space components preserved (1.0^p = 1.0)
         - Non-null components suppressed (0.625^p → 0)
      3. QR the filtered probes
      4. Verify against Cv to get exact null vectors

    Geometric doubling on probe count k: start with k=500, double if
    all probes look null (meaning k < null_dim). Stop when we see the boundary.
    """
    # Step 1: Find lambda_max of Cv
    print(f"    Finding lambda_max...", flush=True)
    t0 = time.time()
    try:
        ev_max = eigsh(Cv_sparse, k=5, which='LM', tol=1e-6, maxiter=3000)[0]
        lambda_max = float(np.max(ev_max)) * 1.05
    except:
        lambda_max = 20.0
    print(f"    lambda_max ≈ {lambda_max:.4f} ({time.time()-t0:.1f}s)", flush=True)

    # Step 2: Build filter matrix B = I - Cv/lambda_max
    B_sparse = sparse.eye(n, format='csr') - Cv_sparse / lambda_max

    # Spectral ratio: non-null eigenvalue of B ≈ 1 - gap/lambda_max
    # For SU(3) Casimir: gap ≈ 3.0, lambda_max ≈ 8.4 → ratio ≈ 0.643
    # After p iterations: ratio^p. Need ratio^p < 1e-14 for clean separation
    gap_estimate = 3.0  # SU(3) Casimir minimum non-zero eigenvalue
    ratio = 1.0 - gap_estimate / lambda_max
    power = max(int(np.ceil(32 / np.log(1 / max(ratio, 0.01)))), 40)
    suppression = ratio ** power
    print(f"    Filter: ratio={ratio:.4f}, power={power}, "
          f"suppression={suppression:.2e}", flush=True)

    # Step 3: Memory-bounded probe count
    bytes_per_col = n * 16  # complex128
    max_probes = int(mem_budget_gb * 1e9 / (2 * bytes_per_col))  # factor 2 for Y + Q
    max_probes = min(max_probes, n)
    print(f"    Max probes for {mem_budget_gb}GB: {max_probes}", flush=True)

    # Step 4: Geometric doubling on probe count
    k = min(500, max_probes, n)
    prev_n_null = -1
    P_null = None

    while k <= min(max_probes, n):
        print(f"    Poly filter: k={k} probes, power={power}...", flush=True)
        t0 = time.time()

        # Random probes (real for Hermitian matrix to save memory)
        np.random.seed(42)
        Y = np.random.randn(n, k) + 1j * np.random.randn(n, k)

        # Apply B^power with periodic QR to maintain numerical stability
        ...
        # Check which columns are in null space of Cv
        CQ = Cv_sparse @ Q
        col_norms = np.linalg.norm(CQ, axis=0)
        del CQ

        # Adaptive threshold: find the gap in sorted col_norms
        sorted_norms = np.sort(col_norms)
        # Look for largest gap in log-space
        log_norms = np.log10(sorted_norms + 1e-20)
        gaps = np.diff(log_norms)
        # The gap between null and non-null should be the largest
        best_gap_idx = np.argmax(gaps)
        adaptive_threshold = 10 ** ((log_norms[best_gap_idx] + log_norms[best_gap_idx + 1]) / 2)
        # Sanity: threshold should be between 1e-10 and 1.0
        adaptive_threshold = np.clip(adaptive_threshold, 1e-10, 1.0)

        null_mask = col_norms < adaptive_threshold
        n_null = int(np.sum(null_mask))
        n_nonzero = Q.shape[1] - n_null

        dt = time.time() - t0
        print(f"    k={k}: {n_null} null, {n_nonzero} non-null ({dt:.1f}s)", flush=True)
        print(f"    Threshold={adaptive_threshold:.2e} "
              f"(gap at idx {best_gap_idx}, "
              f"below={sorted_norms[best_gap_idx]:.2e}, "
              f"above={sorted_norms[best_gap_idx+1]:.2e})", flush=True)

        if n_nonzero > 0:
            # Found boundary: have more probes than null-space dimension
            P_null = Q[:, null_mask]
            print(f"    Boundary found at k={k}: {n_null} null vectors.", flush=True)
            del Q; gc.collect()
            break
        else:
            # All probes are null: k < null_dim, need more probes
            del Q; gc.collect()
            if n_null == prev_n_null:
                # Null count didn't change — might be at the limit
                print(f"    WARNING: null count stalled at {n_null}.", flush=True)
            prev_n_null = n_null
            next_k = min(k * 2, max_probes, n)
            if next_k <= k:
                # Can't increase further — reconstruct from last run
                print(f"    Max probes reached. Re-running with k={k}...", flush=True)
                np.random.seed(42)
                Y = np.random.randn(n, k) + 1j * np.random.randn(n, k)
                for p in range(power):
                    Y = B_sparse @ Y
    ...
    if P_null is None or P_null.shape[1] == 0:
    ...
    # Final verification
    residual = np.max(np.linalg.norm(Cv_sparse @ P_null, axis=0))
    print(f"    Null space: dim={P_null.shape[1]}, "
