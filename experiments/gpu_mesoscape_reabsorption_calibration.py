...
def basis0_fidelity(rho_n):
    return float(np.real(rho_n[0, 0]))


def neighbor_pair_mi_mean(psi, neighbors, n_sites, xp):
    if len(neighbors) < 2:
        return 0.0
    vals = []
    for a, b in combinations(sorted(neighbors), 2):
        vals.append(mutual_information_from_state(psi, a, b, n_sites, xp))
...
class SimConfig:
    ...
    calibrate_every: int
    calibrate_lookahead_windows: int
    cooldown_evals: int
    max_candidates_per_eval: int
    device: str
    progress_every: int
    json_out: str
...
def organizer_role(node, dominant_core):
    if dominant_core is None:
        return "none"
    if node in set(dominant_core["core_pair"]):
        return "core"
    if node in set(dominant_core["shell_nodes"]):
        return "shell"
    return "outside"


def calibrate_reabsorption_candidates(
    psi,
    active_nodes,
    active_edges,
    edge_strengths,
    ...
    core_pair = set(dominant_core["core_pair"])
    organizer_nodes = list(dominant_core["shell_nodes"])
    ...
    for n in active_nodes:
        if n in core_pair:
            continue
        age = eval_index - birth_eval_map.get(n, -10**9)
        if age < cfg.cooldown_evals:
            continue
        rho_n_now = to_cpu_array(partial_trace_keep(psi, [n], cfg.n_max, xp))
        fid0_now = basis0_fidelity(rho_n_now)
        degree = len(adj.get(n, []))
        role = organizer_role(n, dominant_core)
        candidates.append((-(fid0_now), degree, n, age, role, fid0_now))
    candidates.sort()

    out = []
    for _, degree, n, age, role, fid0_now in candidates[: cfg.max_candidates_per_eval]:
        neighbors = sorted(adj.get(n, []))

        active_nodes_test = [x for x in active_nodes if x != n]
        active_edges_test = [e for e in active_edges if n not in e]
        edge_strengths_test = dict(edge_strengths)
        for e in list(edge_strengths_test.keys()):
            if n in e:
                edge_strengths_test.pop(e, None)
        local_coeffs_test = {k: np.array(v, dtype=float).copy() for k, v in local_coeffs.items()}
        local_coeffs_test[n] = np.zeros(8, dtype=float)

        psi_ctrl = evolve_windows(
            psi.copy(),
            cfg.calibrate_lookahead_windows,
            cfg,
            active_nodes,
            active_edges,
            local_coeffs,
        ...
        keep_org = [x for x in organizer_nodes if x != n]
        if keep_org:
            rho_org_ctrl = to_cpu_array(partial_trace_keep(psi_ctrl, keep_org, cfg.n_max, xp))
            rho_org_test = to_cpu_array(partial_trace_keep(psi_test, keep_org, cfg.n_max, xp))
            org_fid = fidelity_uhlmann(rho_org_ctrl, rho_org_test)
            ent_org_ctrl = von_neumann_entropy(np.asarray(rho_org_ctrl), np)
            ent_org_test = von_neumann_entropy(np.asarray(rho_org_test), np)
        else:
            org_fid = 1.0
            ent_org_ctrl = 0.0
            ent_org_test = 0.0

        rho_n_test = to_cpu_array(partial_trace_keep(psi_test, [n], cfg.n_max, xp))
        fid0_test = basis0_fidelity(rho_n_test)

        mi_neighbors_ctrl = neighbor_pair_mi_mean(psi_ctrl, neighbors, cfg.n_max, xp)
        mi_neighbors_test = neighbor_pair_mi_mean(psi_test, neighbors, cfg.n_max, xp)
        if mi_neighbors_ctrl <= 1e-12:
            neighbor_retention = 1.0 if mi_neighbors_test <= 1e-12 else 0.0
        else:
            neighbor_retention = float(mi_neighbors_test / mi_neighbors_ctrl)

        surrogate_score = float((fid0_test * org_fid * max(0.0, neighbor_retention)) ** (1.0 / 3.0))
        ...
        out.append({
            ...
            "basis0_fidelity_now": float(fid0_now),
            "basis0_fidelity_test": float(fid0_test),
            "organizer_fidelity": float(org_fid),
            "organizer_entropy_control": float(ent_org_ctrl),
            "organizer_entropy_test": float(ent_org_test),
            "organizer_entropy_shift": float(ent_org_test - ent_org_ctrl),
            "neighbor_mi_control": float(mi_neighbors_ctrl),
            "neighbor_mi_test": float(mi_neighbors_test),
            "neighbor_mi_retention": float(neighbor_retention),
...
def quantile_summary(vals: List[float]):
    if not vals:
        return {}
...
def build_calibration_summary(records):
    by_role = {}
    for r in records:
    ...
    for name in ["basis0_fidelity_test", "organizer_fidelity", "neighbor_mi_retention", "surrogate_score"]:
        summary["global"][name] = quantile_summary([r[name] for r in records])

    for role, rows in by_role.items():
        summary["by_role"][role] = {}
        for name in ["basis0_fidelity_test", "organizer_fidelity", "neighbor_mi_retention", "surrogate_score"]:
            summary["by_role"][role][name] = quantile_summary([r[name] for r in rows])

    shell_rows = by_role.get("shell", records)
    if shell_rows:
        summary["suggested_thresholds"] = {
            "basis0_fidelity_floor_q90_shell": float(np.quantile([r["basis0_fidelity_test"] for r in shell_rows], 0.90)),
            "organizer_fidelity_floor_q10_shell": float(np.quantile([r["organizer_fidelity"] for r in shell_rows], 0.10)),
            "neighbor_mi_retention_floor_q10_shell": float(np.quantile([r["neighbor_mi_retention"] for r in shell_rows], 0.10)),
            "surrogate_score_floor_q90_shell": float(np.quantile([r["surrogate_score"] for r in shell_rows], 0.90)),
        }
...
def run_sim(cfg: SimConfig):
    ...
    while step < cfg.total_steps:
        ...
        if total_evals % cfg.calibrate_every == 0:
            recs = calibrate_reabsorption_candidates(
                psi=psi,
            ...
            calibration_records.extend(recs)
        ...
        if cfg.progress_every > 0 and total_evals % cfg.progress_every == 0:
            ...
            print(
                ...
                f"cal_records={len(calibration_records)}"
            )
    ...
    summary = {
        ...
        "calibration": build_calibration_summary(calibration_records),
    }
...
def parse_args():
    ap = argparse.ArgumentParser(description="Measure-only calibration for no-forgetting reabsorption thresholds.")
    ...
    ap.add_argument("--calibrate-every", type=int, default=1)
    ap.add_argument("--calibrate-lookahead-windows", type=int, default=2)
    ap.add_argument("--cooldown-evals", type=int, default=2)
    ap.add_argument("--max-candidates-per-eval", type=int, default=6)
    ap.add_argument("--progress-every", type=int, default=1)
    ap.add_argument("--json-out", type=str, default="gpu_mesoscape_reabsorption_calibration.json")
...
def main():
    ...
    cfg = SimConfig(
        ...
        calibrate_lookahead_windows=args.calibrate_lookahead_windows,
        cooldown_evals=args.cooldown_evals,
        max_candidates_per_eval=args.max_candidates_per_eval,
        device=args.device,
    ...
    result = run_sim(cfg)
    print("=" * 96)
    print("REABSORPTION CALIBRATION SUMMARY")
    print("-" * 96)
