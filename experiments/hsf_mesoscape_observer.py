#!/usr/bin/env python3
# filename: hsf_mesoscape_observer.py
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "HSF mesoscale observer. Evolves psi on a fixed lawful substrate setup and "
            "reports MI-first and connected-correlator diagnostics with baseline subtraction, "
            "sector decomposition, and dormant-sector activation detection."
        )
    )
    ...
    # Dormant activation / export diagnostics
    p.add_argument("--activation-threshold-mi", type=float, default=1e-7)
    p.add_argument("--activation-threshold-ccorr", type=float, default=1e-7)
    p.add_argument("--activation-window", type=int, default=5)

    p.add_argument("--json-out", type=str, default="hsf_mesoscape_observer.json")
    return p.parse_args()
...
def longest_positive_run(trace: List[Optional[float]], threshold: float) -> Dict[str, Any]:
    best_len = 0
    best_start = None
    best_end = None
    cur_len = 0
...
def activation_summary(
    delta_mi_trace: List[Optional[float]],
    delta_ccorr_trace: List[Optional[float]],
    mi_threshold: float,
    ccorr_threshold: float,
    window: int,
) -> Dict[str, Any]:
    mi_run = longest_positive_run(delta_mi_trace, mi_threshold)
    cc_run = longest_positive_run(delta_ccorr_trace, ccorr_threshold)
    return {
        "mi_activation_run": mi_run,
        "ccorr_activation_run": cc_run,
        "mi_activation_detected": bool(mi_run["length"] >= window),
        "ccorr_activation_detected": bool(cc_run["length"] >= window),
        "window": int(window),
    }
...
def run_observer(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    export_diagnostics = {
        "delta_ccorr_aa_minus_ad": [],
        "delta_mi_aa_minus_ad": [],
        "delta_ccorr_dd_minus_ad": [],
        "delta_mi_dd_minus_ad": [],
    }
    ...
    for step in range(1, args.total_steps + 1):
        ...
        export_diagnostics["delta_ccorr_aa_minus_ad"].append(
            None if aa_dcc is None or ad_dcc is None else float(aa_dcc - ad_dcc)
        )
        export_diagnostics["delta_mi_aa_minus_ad"].append(
            None if aa_dmi is None or ad_dmi is None else float(aa_dmi - ad_dmi)
        )
        export_diagnostics["delta_ccorr_dd_minus_ad"].append(
            None if dd_dcc is None or ad_dcc is None else float(dd_dcc - ad_dcc)
        )
        export_diagnostics["delta_mi_dd_minus_ad"].append(
            None if dd_dmi is None or ad_dmi is None else float(dd_dmi - ad_dmi)
        )
            ...
            snapshots.append(
                {
                    ...
                    "export_diagnostics": {
                        "delta_ccorr_aa_minus_ad": export_diagnostics["delta_ccorr_aa_minus_ad"][-1],
                        "delta_mi_aa_minus_ad": export_diagnostics["delta_mi_aa_minus_ad"][-1],
                        "delta_ccorr_dd_minus_ad": export_diagnostics["delta_ccorr_dd_minus_ad"][-1],
                        "delta_mi_dd_minus_ad": export_diagnostics["delta_mi_dd_minus_ad"][-1],
                    },
                    "dominant_core": core,
    ...
    summary = {
        ...
        "dormant_activation": {
            "delta_mi_dormant_dormant": activation_summary(
                sector_mean_traces["delta_mi"]["dormant_dormant"],
                sector_mean_traces["delta_ccorr"]["dormant_dormant"],
                args.activation_threshold_mi,
                args.activation_threshold_ccorr,
                args.activation_window,
            )["mi_activation_run"],
            "delta_ccorr_dormant_dormant": activation_summary(
                sector_mean_traces["delta_mi"]["dormant_dormant"],
                sector_mean_traces["delta_ccorr"]["dormant_dormant"],
                args.activation_threshold_mi,
                args.activation_threshold_ccorr,
                args.activation_window,
            )["ccorr_activation_run"],
            "mi_activation_detected": activation_summary(
                sector_mean_traces["delta_mi"]["dormant_dormant"],
                sector_mean_traces["delta_ccorr"]["dormant_dormant"],
                args.activation_threshold_mi,
                args.activation_threshold_ccorr,
                args.activation_window,
            )["mi_activation_detected"],
            "ccorr_activation_detected": activation_summary(
                sector_mean_traces["delta_mi"]["dormant_dormant"],
                sector_mean_traces["delta_ccorr"]["dormant_dormant"],
                args.activation_threshold_mi,
                args.activation_threshold_ccorr,
                args.activation_window,
            )["ccorr_activation_detected"],
            "window": int(args.activation_window),
        },
        "export_diagnostics": {
            key: safe_trace_summary(export_diagnostics[key], args.dt)
            for key in export_diagnostics
        },
    }

    return {
        "script": "hsf_mesoscape_observer.py",
        "observer_mode": True,
        ...
        "observer_config": {
            ...
            "activation_threshold_mi": float(args.activation_threshold_mi),
            "activation_threshold_ccorr": float(args.activation_threshold_ccorr),
            "activation_window": int(args.activation_window),
            "principle": "observe the substrate without choosing births or pruning; use connected correlators, baseline subtraction, sector decomposition, and dormant-sector activation tests to separate initial-condition residue from dynamics",
        },
