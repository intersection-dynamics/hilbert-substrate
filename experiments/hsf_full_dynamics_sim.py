# hsf_full_dynamics_sim.py
# ------------------------------------------------------------
# Hilbert Substrate Framework: Full Dynamics Demonstrator
#
# Starts with an "undifferentiated" substrate:
#   - N nodes (emergent locality candidates)
#   - a random sparse graph (no geometry, no lattice)
#   - random complex amplitudes (no built-in particle notion)
#
# Applies four constraints operationally:
#   1) No-signaling: strictly local updates along active edges each tick
#   2) Finite bandwidth: per-edge transport magnitude capped each tick
#   3) No-forgetting: per-edge memory registers record transported magnitude
#   4) No-refolding: edges become "locked" once memory exceeds a threshold;
#                   locked edges cannot be rewired, constraining reachable topology
#
# Additionally:
#   - Topology adapts early via rewiring pressure under bandwidth+memory.
#   - A small self-focusing term allows localized excitations to form/persist.
#   - Random "kicks" seed disturbances so we can watch defect/excitation ecology.
#
# Outputs:
...
#     plots/
#       graph_metrics.png
#       support_excitation.png
#       memory_concentration.png
#       dimension_estimate.png
#
# Windows one-liner example:
#   python hsf_full_dynamics_sim.py --N 384 --T 1200 --k0 10 --target_deg 6 --bandwidth 0.02 --alpha 0.14 --kappa 8 --eta 0.8 --rewire_every 6 --rewire_budget 20
# ------------------------------------------------------------
...
def count_excitation_peaks(weights: np.ndarray, adj: List[np.ndarray], thresh: float) -> int:
    # Count nodes above thresh that are strict local maxima among neighbors
    w = weights
    peaks = 0
...
def build_random_k_regular_graph(N: int, k: int, rng: np.random.Generator) -> Tuple[np.ndarray, np.ndarray]:
    """
    Build an undirected random k-regular-ish graph via stub matching.
    Returns edge arrays (u, v) with u < v.
    """
    if k >= N:
    ...
    # If not enough edges, fill randomly
    target_m = (N * k) // 2
...
def edges_to_adj(N: int, u: np.ndarray, v: np.ndarray) -> List[np.ndarray]:
    # Build adjacency arrays per node
...
def bfs_ball_sizes(adj: List[np.ndarray], start: int, max_r: int) -> np.ndarray:
    # returns count within r hops for r=1..max_r
    N = len(adj)
...
def estimate_effective_dimension(adj: List[np.ndarray], rng: np.random.Generator, samples: int = 12, max_r: int = 6) -> float:
    # Estimate d from growth ~ r^d using median over samples and linear fit in log space
    N = len(adj)
    ...
    # fit log V = d log r + c over r=2..max_r (skip r=1 which can be noisy)
    x = np.log(rs[1:])
    y = med_logV[1:]
...
def run_sim(p: SimParams) -> dict:
    ...
    # Undifferentiated initial topology: random k0-regular-ish graph
    u, v = build_random_k_regular_graph(p.N, p.k0, rng)
    m = np.zeros(u.size, dtype=np.float64)          # no-forgetting memory per edge
    locked = np.zeros(u.size, dtype=np.int8)        # no-refolding lock flag per edge

    # Undifferentiated initial state: random complex vector
    ...
    # Store a few snapshots of |psi|^2
    snap_times = {0, steps // 4, steps // 2, (3 * steps) // 4, steps - 1}
    snap_t = []
    snap_w = []

    for t in range(steps):
        # rebuild adjacency view for local operations (no-signaling)
        adj = edges_to_adj(p.N, u, v)

        # ---- Local evolution (no-signaling + bandwidth) ----
        dpsi = np.zeros_like(psi)
        flow_mag = np.zeros(u.size, dtype=np.float64)

        # random "kicks" to seed disturbances / excitations
        ...
        for ei, (a, b) in enumerate(zip(u, v)):
            ...
            # memory throttling (no-refolding flavor even before lock)
            throttle = np.exp(-p.kappa * m[ei]) if p.kappa > 0 else 1.0
            flow = p.alpha * throttle * grad

            # finite bandwidth per edge
            mag = np.abs(flow)
            if mag > p.bandwidth:
                flow *= (p.bandwidth / (mag + 1e-12))
                mag = p.bandwidth

            # apply antisymmetric transport (local only)
            dpsi[a] -= flow
            dpsi[b] += flow
            flow_mag[ei] = float(mag)

            # no-forgetting bookkeeping: memory records transport
            transported = float(mag)
            m[ei] = (1.0 - p.mem_decay) * m[ei] + p.mem_coupling * transported

            # no-refolding lock: once memory exceeds threshold, freeze this edge
            if locked[ei] == 0 and m[ei] >= p.lock_threshold:
                locked[ei] = 1

        # Self-focusing (helps “lumps” form; think of it as compressive pressure)
        # This is deliberately tiny and renormalized to stay “unitary-ish”.
        if p.eta != 0.0:
            amp2 = (np.abs(psi) ** 2)
            psi = psi * np.exp(p.eta * (amp2 - amp2.mean()))

        # update + renormalize
        psi = psi + dpsi
        nrm = np.sqrt(np.vdot(psi, psi).real + 1e-12)
        psi /= nrm
        ...
        # peak count (excitation count proxy) based on amplitude support
        # threshold relative to mean weight
        ...
        # ---- Topology adaptation (refolding allowed early, forbidden as edges lock) ----
        # This is where "space" emerges: we prune low-use unlocked edges and add new ones
        # that align with existing correlations, under bandwidth pressure.
        if (p.rewire_every > 0) and (t % p.rewire_every == 0) and (t < p.warmup_steps):
            # Score edges for removal: low flow, unlocked, and low memory (not important)
            removable = np.where(locked == 0)[0]
            if removable.size > 0:
                scores = flow_mag[removable] + 0.15 * np.abs(m[removable])
                # remove the worst edges (lowest scores)
            ...
            # Add edges back to push toward target degree, guided by current correlations.
            # Correlation proxy: c_ij = |psi_i * conj(psi_j)| = |psi_i||psi_j|
            # BUT selection is constrained by *existing neighborhood structure*:
            # we propose edges between nodes that already share neighbors (triadic closure),
            # which is a purely graph-local move and can generate locality-like clustering
            # without assuming coordinates.
            adj = edges_to_adj(p.N, u, v)
            deg = np.array([a.size for a in adj], dtype=np.int32)
            need = np.where(deg < p.target_deg)[0]
            add_budget = p.rewire_budget

            # For speed, use a set for edge existence checks
            edge_set = set()
            for a, b in zip(u, v):
            ...
            for i in need[: min(need.size, add_budget)]:
                # Candidate partners:
                # pick a neighbor-of-neighbor (triadic closure) or fallback random
                if adj[i].size > 0:
                    j = int(rng.choice(adj[i]))
                    if adj[j].size > 0:
                        k = int(rng.choice(adj[j]))
                    else:
                ...
                # Correlation-guided acceptance: prefer connecting higher |psi| nodes (bookkeeping hubs)
...
def plot_all(run_dir: str, data: dict) -> None:
    ...
    if snap_w.size > 0:
        ...
        plt.xlabel("site index (no geometry assumed)")
        plt.ylabel("|psi|^2")
        plt.legend()
        plt.savefig(os.path.join(plots_dir, "snapshots.png"), dpi=160, bbox_inches="tight")
        plt.close()
...
def main():
    ...
    ap.add_argument("--k0", type=int, default=10, help="initial random degree (undifferentiated graph)")
    ap.add_argument("--target_deg", type=int, default=6, help="target mean degree after rewiring")
    ...
    ap.add_argument("--mem_decay", type=float, default=0.001)
    ap.add_argument("--mem_coupling", type=float, default=1.0)
    ap.add_argument("--lock_threshold", type=float, default=1.8)

    ap.add_argument("--eta", type=float, default=0.8, help="self-focusing strength (helps localized excitations)")
    ap.add_argument("--rewire_every", type=int, default=6)
    ap.add_argument("--rewire_budget", type=int, default=20)
    ap.add_argument("--warmup_steps", type=int, default=500, help="rewiring only before this tick (then topology freezes)")

    ap.add_argument("--kick_every", type=int, default=40)
    ...
    tag = (
        ...
        f"_eta{args.eta}_lock{args.lock_threshold}"
