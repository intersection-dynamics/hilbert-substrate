...
def candidate_features(psi, active_nodes, active_edges, edge_strengths, n_sites, GM, xp, cfg):
    ...
    for i, j in combinations(active_nodes, 2):
        ...
        rows.append({
            ...
            "common_nbrs": [int(x) for x in common_nbrs],
            "cmi_mean": cmi_mean,
...
def choose_candidate_births(rows, dormant_nodes, candidate_fraction, fission_fraction, birth_score_floor, max_births_per_eval, birth_exclusion_radius):
    if not rows or not dormant_nodes:
        return []

    n_considered = max(1, int(np.ceil(candidate_fraction * len(rows))))
    considered = [r for r in rows[:n_considered] if float(r["score"]) >= float(birth_score_floor)]
    if not considered:
        return []

    n_target = max(1, int(np.ceil(fission_fraction * len(considered))))
    n_target = min(n_target, len(considered), len(dormant_nodes), int(max_births_per_eval))
    if n_target <= 0:
        return []

    chosen_rows = []
    used_parent_nodes = set()
    used_support_nodes = set()
    used_centers = []

    for row in considered:
        i, j = row["pair"]
        pair_nodes = {int(i), int(j)}
        support_nodes = pair_nodes.union(int(x) for x in row.get("common_nbrs", []))
        center = float(row.get("pair_center", 0.0))

        if pair_nodes & used_parent_nodes:
            continue
        if support_nodes & used_support_nodes:
            continue
        if any(abs(center - c) < float(birth_exclusion_radius) for c in used_centers):
            continue

        chosen_rows.append(row)
        used_parent_nodes.update(pair_nodes)
        used_support_nodes.update(support_nodes)
        used_centers.append(center)

        if len(chosen_rows) >= n_target:
            break

    if not chosen_rows:
        chosen_rows = [considered[0]]

    n_assign = min(len(chosen_rows), len(dormant_nodes), int(max_births_per_eval))
    return [(chosen_rows[idx], dormant_nodes[idx]) for idx in range(n_assign)]
...
def run_sim(cfg: SimConfig):
    ...
    while step < cfg.total_steps:
        ...
        chosen = choose_candidate_births(
            rows,
            dormant_nodes,
            cfg.candidate_fraction,
            cfg.fission_fraction,
            cfg.birth_score_floor,
            cfg.max_births_per_eval,
            cfg.birth_exclusion_radius,
        )
        ...
        snapshots.append({
            ...
            "dominant_core": core,
            "metric": metric,
            "birth_candidates_this_window": int(len(rows)),
...
def parse_args():
    ...
    ap.add_argument("--max-births-per-eval", type=int, default=3)
    ap.add_argument("--birth-exclusion-radius", type=float, default=0.75)
...
def main():
    ...
    cfg = SimConfig(
        ...
        max_births_per_eval=args.max_births_per_eval,
        birth_exclusion_radius=args.birth_exclusion_radius,
