...
def build_site_generator_cache(n: int) -> Dict[Tuple[int,int], np.ndarray]:
    cache = {}
    for site in range(n):
        for a, lam in enumerate(GM):
            ops = [I3] * n
            ops[site] = lam
            cache[(site, a)] = kron_all(ops)
    return cache

def build_hamiltonian(n: int, edges: List[Tuple[int,int]], cache: Dict[Tuple[int,int], np.ndarray], rng: np.random.Generator, local_scale: float, pair_scale: float):
    dim = 3 ** n
    H = np.zeros((dim, dim), dtype=complex)
    for site in range(n):
        coeffs = rng.uniform(-local_scale, local_scale, size=8)
        for a in range(8):
            H += float(coeffs[a]) * cache[(site, a)]
    edge_strengths = {}
    for e in edges:
        i, j = e
        ...
        edge_strengths[e] = g
        for a in range(8):
            H += g * (cache[(i, a)] @ cache[(j, a)])
    return 0.5 * (H + H.conj().T), edge_strengths
...
def random_product_qutrit_state(n: int, seed: int) -> np.ndarray:
    ...
    for _ in range(n):
        ...
        local_states.append(z / np.linalg.norm(z))
    psi = local_states[0]
    for v in local_states[1:]:
        psi = np.kron(psi, v)
    return normalize_state(psi)

def pair_su3_correlator_strength(psi: np.ndarray, cache: Dict[Tuple[int,int], np.ndarray], i: int, j: int) -> float:
    vals = []
    for a in range(8):
        vals.append(float(np.real(expect(psi, cache[(i, a)] @ cache[(j, a)]))))
    return float(np.linalg.norm(vals))

def analyze_pairs(psi: np.ndarray, rho: np.ndarray, n: int, dims: List[int], cache: Dict[Tuple[int,int], np.ndarray], edges: List[Tuple[int,int]]) -> List[Dict[str, object]]:
    out = []
    for (i, j) in edges:
...
def analyze_triangles(rho: np.ndarray, n: int, dims: List[int], edges: List[Tuple[int,int]]) -> List[Dict[str, object]]:
    ...
    for a, b, c in combinations(range(n), 3):
        ...
        if eab in edge_set and ebc in edge_set and eca in edge_set:
            ...
            out.append({
                "triangle": [a, b, c],
                "mi_ab": mi_ab, "mi_bc": mi_bc, "mi_ca": mi_ca,
                "pair_mi_mean": pair_mean, "pair_mi_std": imbalance,
...
def run_sim(cfg: SimConfig) -> Dict[str, object]:
    if cfg.n > 6:
        raise ValueError("This exact full-subsystem qutrit script gets very heavy quickly. Use N <= 6 for practical runs.")
    ...
    U = matrix_exp_hermitian(H, cfg.time)
    psi = normalize_state(U @ psi0)
...
def pretty_report(result: Dict[str, object]) -> str:
    ...
    lines = []
    lines.append("=" * 108)
    lines.append("HSF FULL-HILBERT SUBSYSTEMS SU(3) ANALYSIS (v1)")
    lines.append("-" * 108)
    ...
    lines.append("Subsystems are full qutrit Hilbert factors. Links are SU(3)-style pair couplings between those full subsystem factors.")
    lines.append("-" * 108)
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Exact full-Hilbert qutrit subsystem SU(3) analyzer.")
