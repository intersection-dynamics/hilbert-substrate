...
def plot_results(all_results, critical_alphas, output_dir=None):
    """Generate publication-quality plots of binding results."""
    from pathlib import Path
    if output_dir is None:
        output_dir = Path('hsf_out') / 'matter_stability'
...
if __name__ == "__main__":
    ...
    parser.add_argument('--out_dir', type=str, default=None,
                        help='Base output directory (default: <repo>/hsf_out/matter_stability/<timestamp>)')
    parser.add_argument('--no_save', action='store_true',
                        help='Do not write JSON results to disk')
    ...
    # experiments/.. => repo root assumed to be parent of "experiments"
    repo_root = here.parent.parent if here.parent.name.lower() == "experiments" else here.parent
    base_out = Path(args.out_dir) if args.out_dir else (repo_root / "hsf_out" / "matter_stability")
    run_id = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    run_dir = base_out / run_id
    run_dir.mkdir(parents=True, exist_ok=True)

    # Default JSON output path if not explicitly provided
    if args.output is None:
        args.output = str(run_dir / "results.json")
    
    start_time = time.time()
    print(f"Run directory: {run_dir}")
    print(f"JSON output:   {args.output}" if not args.no_save else "JSON output:   (disabled)")
    ...
    if args.quick:
        ...
        try:
            plot_results(all_results, critical_alphas, output_dir=run_dir)
