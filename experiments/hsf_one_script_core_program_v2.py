#!/usr/bin/env python3
"""
hsf_one_script_core_program_v2.py

Single-file, shareable, end-to-end HSF demonstrator (distilled):

A) Emergence of spatial locality
   - Build a pregeometric influence/accessibility kernel over "sites"
   - Apply constraint-shaped flow (finite bandwidth, no-forgetting, no-refolding, no-signaling proxy)
   - Produce spectral embedding + simple locality metrics that should IMPROVE under constraints

B) Emergence of factorization (bidirectional link structure)
   - Construct a true (dL ⊗ dR) operator with explicit L/R structure
   - Scramble it by a global unitary (hides the factorization)
   - RECOVER factorization by searching for a basis W that minimizes operator-Schmidt complexity
     for candidate (dL,dR). Pick dims with best objective.

Dependencies: numpy, scipy, matplotlib

Windows one-liners:
  python hsf_one_script_core_program_v2.py --outdir hsf_one_script_out_v2
  python hsf_one_script_core_program_v2.py --outdir out --seed 0 --n_sites 27 --steps 450 --bandwidth_k 8
  python hsf_one_script_core_program_v2.py --outdir out --dL 4 --dR 4 --schmidt_iters 300 --schmidt_rank 3
"""
...
def expm_unitary_from_antiherm(K: np.ndarray) -> np.ndarray:
    # K anti-Hermitian => exp(K) is unitary
    return la.expm(K)


# ----------------------------
# Part A: Locality via constrained influence flow
# ----------------------------
...
def initial_influence_kernel(n_sites: int, seed_scale: float = 1.0) -> np.ndarray:
    # Start with random symmetric weights, then keep only positive part for a clean Laplacian
...
def spectral_bandwidth_truncate_pos(a: np.ndarray, k: int) -> np.ndarray:
    # Truncate eigenmodes of the *symmetric* matrix; then project back to nonnegative part
    w, v = la.eigh(a)
    ...
    if np.max(a2) > 0:
        a2 = a2 / (np.max(a2) + 1e-12) * (np.max(a) + 1e-12)
    return a2

def embed_from_kernel(a: np.ndarray, dim: int = 3) -> np.ndarray:
    # Laplacian eigenmap from nonnegative symmetric adjacency
...
def locality_metrics_from_embedding(coords: np.ndarray, k: int = 4) -> LocalityMetrics:
    ...
    lattice = clip01(0.65 * float(recip) + 0.35 * (1.0 - clip01(cv)))
    return LocalityMetrics(float(recip), float(cv), float(lattice))
...
def constrained_flow(
    ...
    """
    Key changes vs v1:
      - A is kept NONNEGATIVE and symmetric throughout (stable Laplacian embedding)
      - no-signaling proxy aligns A toward a distance kernel softly (no sign flips)
      - no-refolding penalizes departures from smoothed history
    """
    A = A0.copy()
    A_hist = A0.copy()
    ...
    for _ in range(steps):
        ...
        # embed current A to define emergent distances
        coords = embed_from_kernel(A, dim=3)

        # no-signaling proxy: influence decays with emergent distance
        if weights.no_signaling > 0:
            ...
            # scale K to current max(A) and mix gently
            K = K * (np.max(A) + 1e-12)
            lam = signal_mix * weights.no_signaling
            lam = float(max(0.0, min(0.6, lam)))  # clamp
            A = (1 - lam) * A + lam * K

        # no-refolding proxy: resist rapid rewiring vs history
        ...
        m = np.max(A)
        if m > 0:
            A = A / (m + 1e-12) * (np.max(A0) + 1e-12)

    return A


# ----------------------------
# Part B: Factorization emergence via basis search for low operator-Schmidt complexity
# ----------------------------
...
class FactorizationDiagnostics:
    ...
    objective: float
    relative_fit_error_rankR: float
...
def kron_rankR_approx_error(M: np.ndarray, dL: int, dR: int, R: int) -> float:
    """
    Best rank-R Kronecker sum approximation error using top-R SVD terms of reshuffle.
      M ≈ sum_{r=1..R} kron(A_r, B_r)
    """
    D = dL * dR
    T = M.reshape(dL, dR, dL, dR).transpose(0, 2, 1, 3).reshape(dL*dL, dR*dR)
...
def scramble_factorization_operator(dL: int, dR: int, seed_scale: float = 1.0) -> Tuple[np.ndarray, np.ndarray]:
    """
    Build an operator with explicit L/R structure, then scramble it by global unitary.
    """
...
def schmidt_objective(M: np.ndarray, dL: int, dR: int, rank_keep: int) -> Tuple[float, List[float]]:
    """
    Objective = fraction of Schmidt weight beyond the first 'rank_keep' singular values.
    Smaller is better (more factorable / bidirectional).
    """
...
def basis_search_low_schmidt(
    ...
    """
    Stochastic hill-climb over unitary W to minimize Schmidt tail objective of W† H W.
    """
    D = dL * dR
    best_obj = None
    best_W = None
    ...
    for r in range(restarts):
        ...
        M = W.conj().T @ H @ W
        obj, top = schmidt_objective(M, dL, dR, rank_keep)

        for _ in range(iters):
            # random anti-Hermitian perturbation
            X = (np.random.randn(D, D) + 1j*np.random.randn(D, D)) / math.sqrt(2.0)
            X = X - X.conj().T  # anti-Hermitian
            K = step_scale * X / (frob(X) + 1e-12)

            U = expm_unitary_from_antiherm(K)
            W_try = W @ U
            M_try = W_try.conj().T @ H @ W_try
            obj_try, top_try = schmidt_objective(M_try, dL, dR, rank_keep)
    ...
    assert best_W is not None and best_obj is not None and best_top is not None
...
def recover_factorization(
    ...
    """
    For each candidate (dL,dR):
      - search basis W minimizing Schmidt tail objective for W† H W
      - compute rank-R Kronecker-sum approximation error in that best basis
    Pick dims with best objective (tie-break by lower Kron error).
    """
    best = None
    ...
    for dL, dR in candidates:
        ...
        if best is None or key < best[0]:
            best = (key, FactorizationDiagnostics(
                inferred_dims=(dL, dR),
                objective=float(obj),
                relative_fit_error_rankR=float(rel),
...
def main() -> int:
    ...
    ap.add_argument("--outdir", type=str, default="hsf_one_script_out_v2")
    ...
    ap.add_argument("--bandwidth_k", type=int, default=6)
    ap.add_argument("--memory_alpha", type=float, default=0.08)
    ap.add_argument("--refold_beta", type=float, default=0.85)
    ap.add_argument("--signal_mix", type=float, default=0.18, help="Gentle mix for signaling alignment (0..0.6).")
    ...
    ap.add_argument("--schmidt_rank", type=int, default=3, help="How many Schmidt components to keep as 'simple'.")
    ap.add_argument("--schmidt_iters", type=int, default=250, help="Hill-climb iterations per restart per candidate.")
    ap.add_argument("--schmidt_step", type=float, default=0.18, help="Perturbation step scale.")
    ap.add_argument("--schmidt_restarts", type=int, default=2, help="Random restarts per candidate dims.")
    ap.add_argument("--kron_rank", type=int, default=3, help="Rank-R Kronecker-sum approximation used for error.")
    ...
    plot_influence_matrix(A,  outdir / "figures" / "locality_A_constrained.png",
                          "Constrained influence (post-flow, nonnegative)")
    ...
    # ---- B) Factorization
    H_true, H_scr = scramble_factorization_operator(args.dL, args.dR, seed_scale=1.0)
    D = H_scr.shape[0]
    ...
    plot_singular_values(
        ...
        f"Top Schmidt SVs (best dims={diag.inferred_dims[0]}x{diag.inferred_dims[1]}, obj={diag.objective:.3f})"
    )
    ...
    summary: Dict[str, Any] = {
        ...
        "factorization": {
            ...
            "objective_tail_fraction": float(diag.objective),
            "kron_rank": int(args.kron_rank),
            "relative_kron_fit_error_rankR": float(diag.relative_fit_error_rankR),
    ...
    print("\n=== HSF one-script core program (v2) ===")
    ...
    print(f"  schmidt-tail objective (lower=better): {diag.objective:.6f}")
    print(f"  rel_kron_fit_error_rank{args.kron_rank}: {diag.relative_fit_error_rankR:.6f}")
