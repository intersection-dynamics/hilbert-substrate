...
def summarize_epochs(snapshots):
    ...
    epochs.append({
        ...
        "lifetime": len(snapshots) - start_idx,
    })

    lifetimes = [e["lifetime"] for e in epochs]
    longest = max(epochs, key=lambda e: e["lifetime"]) if epochs else None
    return {
        "core_switch_count": max(0, len(epochs) - 1),
        "longest_lived_core": longest["core_pair"] if longest else None,
        "longest_core_lifetime": int(longest["lifetime"]) if longest else 0,
        "mean_core_lifetime": float(np.mean(lifetimes)) if lifetimes else 0.0,
        "metastable_epochs": epochs,
    }
...
def run_sim(cfg: SimConfig):
    ...
    for i, j in combinations(active_nodes, 2):
        ...
        edge_strengths[e] = float(rng.uniform(0.7 * cfg.pair_scale, 1.3 * cfg.pair_scale))

    local_states = []
    for i in range(cfg.n_max):
        if i in active_nodes:
            vec = rng.normal(size=3) + 1j * rng.normal(size=3)
            vec = vec / np.linalg.norm(vec)
            local_states.append(xp.asarray(vec, dtype=xp.complex128))
        else:
...
def pretty_report(result):
    ...
    lines = []
    lines.append("=" * 112)
    lines.append("GPU MESOSCAPE QUANTUM-LABEL ANALYSIS (v1)")
    lines.append("-" * 112)
    lines.append(
    ...
    for snap in result["snapshots"][:12]:
        ...
        if core is None or labels is None:
            lines.append(f"  step={snap['step']}  no_core  births={snap['births_this_window']}  extent={snap['metric']['metric_extent']:.4f}")
        else:
            cart = labels["cartan"]["shell"]
            ent = labels["entanglement"]
            topo = labels["topology"]
            lines.append(
                f"  step={snap['step']}  core={core['core_pair']}  shell_size={core['shell_size']}  "
                f"Q3={cart['Q3']:.4f}  Q8={cart['Q8']:.4f}  |Q|={cart['radius']:.4f}  "
                f"S_shell={ent['shell_entropy']:.4f}  frust={topo['frustration_proxy']:.4f}  "
                f"births={snap['births_this_window']}  extent={snap['metric']['metric_extent']:.4f}  changed={snap['core_changed']}"
            )
    return "\n".join(lines)


def parse_args():
    ap = argparse.ArgumentParser(description="GPU mesoscape quantum-label analyzer built on the calibrated mesoscape baseline.")
    ...
    ap.add_argument("--top-spectrum-k", type=int, default=8)
...
def main():
    ...
    cfg = SimConfig(
        ...
        top_spectrum_k=args.top_spectrum_k,
        device=args.device,
