#!/usr/bin/env python3
# filename: hsf_factorization_native_observer.py
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "HSF factorization-native observer. Each candidate factorization is evolved in its own "
            "native tensor-product language with a dimension-matched local operator basis, then "
            "scored using factorization-fair accessibility metrics."
        )
    )
    p.add_argument("--seed", type=int, default=3)
    p.add_argument("--total-dimension", type=int, default=81)
    ...
    p.add_argument(
        ...
        type=str,
        default="",
        help='Optional explicit factorizations like "3x3x3x3;3x3x9;9x9;3x27".',
    )
    ...
    p.add_argument("--json-out", type=str, default="hsf_factorization_native_observer.json")
    return p.parse_args()


def build_native_config(args: argparse.Namespace) -> NativeConfig:
    return NativeConfig(
...
def generalized_gell_mann(d: int) -> List[np.ndarray]:
    mats: List[np.ndarray] = []
    # symmetric and antisymmetric
    for i in range(d):
        for j in range(i + 1, d):
            sym = np.zeros((d, d), dtype=np.complex128)
            asym = np.zeros((d, d), dtype=np.complex128)
            sym[i, j] = sym[j, i] = 1.0
            asym[i, j] = -1.0j
            asym[j, i] = 1.0j
            mats.append(sym)
    ...
    for k in range(1, d):
        ...
        coeff = np.sqrt(2.0 / (k * (k + 1)))
        for i in range(k):
            mat[i, i] = 1.0
        mat[k, k] = -float(k)
        mats.append(coeff * mat)
    return mats


def apply_one_body_local(psi: np.ndarray, op: np.ndarray, axis: int) -> np.ndarray:
    moved = np.moveaxis(psi, axis, 0)
    rest = moved.shape[1:]
    flat = moved.reshape(op.shape[1], -1)
    out = op @ flat
    out = out.reshape((op.shape[0],) + rest)
    return np.moveaxis(out, 0, axis)


def local_hamiltonian_apply(
    psi: np.ndarray,
    local_ops: List[List[np.ndarray]],
    local_scale: float,
    pair_scale: float,
) -> np.ndarray:
    n = psi.ndim
    ...
    # one-body native local terms: average of local generators
    for i in range(n):
        ops_i = local_ops[i]
        if not ops_i:
            continue
        accum = np.zeros_like(psi, dtype=np.complex128)
        for op in ops_i:
            accum += apply_one_body_local(psi, op, i)
        out += local_scale * (accum / len(ops_i))

    # pair native coupling: overlap over min(len(ops_i), len(ops_j))
    for i in range(n):
        for j in range(i + 1, n):
            ops_i = local_ops[i]
            ops_j = local_ops[j]
            m = min(len(ops_i), len(ops_j))
            if m == 0:
                continue
            accum = np.zeros_like(psi, dtype=np.complex128)
            for k in range(m):
                tmp = apply_one_body_local(psi, ops_j[k], j)
                tmp = apply_one_body_local(tmp, ops_i[k], i)
                accum += tmp
            out += pair_scale * (accum / m)
...
def rk4_step_native(
    ...
    def f(x: np.ndarray) -> np.ndarray:
        return -1.0j * local_hamiltonian_apply(x, local_ops, local_scale, pair_scale)
    ...
    norm = np.linalg.norm(out.ravel())
    if norm > 0.0:
        out = out / norm
    return out


def snapshot_for_native_state(psi: np.ndarray, dims: FactorDims, step: int, max_pairs_in_grid: int) -> Dict[str, Any]:
    single_ent = single_entropies_for_factorization(psi)
    mi = pair_mi_for_factorization(psi, single_entropies=single_ent)
    ...
    return {
        ...
        "core_persistence_score": 0.0,
    }


def score_factorization(snapshots: List[Dict[str, Any]], cfg: NativeConfig) -> Dict[str, Any]:
...
def main() -> None:
    args = parse_args()
    native_cfg = build_native_config(args)

    explicit = parse_factorizations(args.candidate_factorizations)
    factorizations = explicit if explicit else all_factorizations_of_dim(int(args.total_dimension))
    ...
    for dims in factorizations:
        if int(np.prod(dims)) != int(args.total_dimension):
            raise ValueError(f"Factorization {dims} does not match total dimension {args.total_dimension}")

        psi = make_initial_state(dims, args.initial_state, args.seed, args.perturb_eps)
        local_ops = [generalized_gell_mann(int(d)) for d in dims]
        ...
        for step in range(1, args.total_steps + 1):
            ...
            if step % args.snapshot_every == 0 or step == 1 or step == args.total_steps:
                snap = snapshot_for_native_state(psi, dims, step, max_pairs_in_grid)
                snaps.append(snap)
            ...
            if args.progress_every > 0 and step % args.progress_every == 0:
                print(f"[{ 'x'.join(str(x) for x in dims) }] step {step:04d}")

        num_pairs = int(snaps[0]["num_pairs"]) if snaps else 0
        persistence = core_persistence_score(core_pairs, num_pairs)
        for snap in snaps:
            snap["core_persistence_score"] = persistence

        summary = score_factorization(snaps, native_cfg)
    ...
    result = {
        "script": "hsf_factorization_native_observer.py",
        "native_config": asdict(native_cfg),
        "initial_state": args.initial_state,
        "perturb_eps": float(args.perturb_eps),
        "total_dimension": int(args.total_dimension),
