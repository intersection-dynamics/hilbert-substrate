...
"""
subsystem_emission_budget_su3_v4.py

HSF subsystem-emission experiment (SU(3), v4)

Goal
----
Differentiate, direction by direction, between:
- dark content on B (no attached link can see it),
- visible content that is actually emitted,
- visible content that remains in B (un-emitted).

This version closes the accounting coordinate-by-coordinate in the
8-dimensional traceless SU(3) operator-coordinate space on B.
"""
...
def analyze_case(case: str, seed: int, poke_idx: int, delta: float, time: float,
    ...
    for j in range(8):
        ...
        if vis[j]["kind"] == "dark":
            dark_coeffs[j] = cB[j]
            coord_rows.append({
                "coord": j, "lambda_label": f"lambda_{j+1}", "kind": "dark", "links": [],
                "available_abs": available, "requested_total": 0.0, "emitted_total": 0.0,
                "visible_unemitted_abs": 0.0, "dark_abs": available,
                "budget_before": budget_remaining, "budget_after": budget_remaining,
                "per_link_request": {name: 0.0 for name in link_names},
                "per_link_emitted": {name: 0.0 for name in link_names},
            })
            continue
        budget_before = budget_remaining
        alloc, emitted_total = allocate_one_direction(
            amp=available, requests=req, priorities=priorities,
            budget_remaining=budget_remaining, mode=allocation
        )
        ...
        coord_rows.append({
            "coord": j, "lambda_label": f"lambda_{j+1}", "kind": vis[j]["kind"],
            "links": visible_links, "available_abs": available,
            "requested_total": float(np.sum(req)), "emitted_total": emitted_total,
            "visible_unemitted_abs": visible_unemitted, "dark_abs": 0.0,
            "budget_before": budget_before, "budget_after": budget_remaining,
            "per_link_request": {link_names[i]: float(req[i]) for i in range(nL)},
            "per_link_emitted": {link_names[i]: float(alloc[i]) for i in range(nL)},
        })
    ...
    per_link_rows = []
    for i, lk in enumerate(links):
        ex_req = float(np.sum([requested_by_link_coord[i, j] for j in exclusive_idx]))
        sh_req = float(np.sum([requested_by_link_coord[i, j] for j in shared_idx]))
        ex_emit = float(np.sum([emitted_by_link_coord[i, j] for j in exclusive_idx]))
        sh_emit = float(np.sum([emitted_by_link_coord[i, j] for j in shared_idx]))
        per_link_rows.append({
            "name": lk.name, "priority": float(lk.priority), "basis_idx": list(lk.basis_idx),
            "requested_total": float(np.sum(requested_by_link_coord[i])),
            "emitted_total": float(emitted_total_by_link[i]),
            "exclusive_requested": ex_req, "shared_requested": sh_req,
            "exclusive_emitted": ex_emit, "shared_emitted": sh_emit,
            "requested_by_coord": [float(x) for x in requested_by_link_coord[i].tolist()],
            "emitted_by_coord": [float(x) for x in emitted_by_link_coord[i].tolist()],
        })
    ...
    for row in coord_rows:
        gap = row["available_abs"] - (row["dark_abs"] + row["emitted_total"] + row["visible_unemitted_abs"])
        per_coord_gap.append(float(gap))
    max_abs_gap = float(max(abs(g) for g in per_coord_gap)) if per_coord_gap else 0.0
    ...
    return {
        "case": case, "seed": int(seed), "poke_idx": int(poke_idx), "time": float(time),
        "delta": float(delta), "budget_frac": float(budget_frac), "budget_abs": float(budget_abs),
        "budget_remaining_final": float(budget_remaining), "allocation": allocation,
        "coupling_profile": coupling_profile, "B_coeffs": [float(x) for x in cB.tolist()],
        "B_absorb_norm": total_absorb, "B_absorb_rank": effective_rank_from_coeffs(cB),
        ...
        "norms": {
            "dark_norm": dark_abs, "visible_norm": visible_abs,
            "emitted_visible_norm": emitted_abs, "visible_unemitted_norm": visible_unemitted_abs,
        ...
        "coords": coord_rows, "links": per_link_rows,
        "derived_coeffs": {
            "dark_coeffs": [float(x) for x in dark_coeffs.tolist()],
            "visible_coeffs": [float(x) for x in visible_coeffs.tolist()],
            "emitted_visible_coeffs": [float(x) for x in emitted_visible_coeffs.tolist()],
            "visible_unemitted_coeffs": [float(x) for x in visible_unemitted_coeffs.tolist()],
            "retained_coeffs": [float(x) for x in retained_coeffs.tolist()],
        },
    }
...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="HSF subsystem emission-budget witness on SU(3) operator content (v4).")
...
def main() -> int:
    args = parse_args()
    all_res = []
    print("\\nHSF SUBSYSTEM EMISSION-BUDGET TEST (SU3 v4)\\n")
    for case in args.cases:
        res = analyze_case(case=case, seed=args.seed, poke_idx=args.poke_idx, delta=args.delta,
                           time=args.time, budget_frac=args.budget_frac, allocation=args.allocation,
