#!/usr/bin/env python3
# filename: hsf_mesoscape_entanglement_batched_birth.py
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "HSF mesoscale entanglement-batched-birth sandbox. "
            "Birth candidates are generated only from entangled parent pairs. "
            "A cheap batched birth proxy ranks all candidates on GPU/CPU, then only top finalists "
            "are scored with exact branch evolution and witnesses."
        )
    )
    ...
    p.add_argument("--birth-topk-finalists", type=int, default=8)
    ...
    p.add_argument("--json-out", type=str, default="hsf_mesoscape_entanglement_batched_birth.json")
    return p.parse_args()
...
def entanglement_sources(psi, sigma: np.ndarray, active_nodes: Set[int], xp, args: argparse.Namespace) -> List[Dict[str, Any]]:
...
def generate_birth_candidates(psi, sigma: np.ndarray, interface_commitment: Dict[Edge, float], active_nodes: Set[int], args: argparse.Namespace, xp):
    parent_rows = entanglement_sources(psi, sigma, active_nodes, xp, args)
...
def batched_birth_proxy(candidates: List[Dict[str, Any]], sigma: np.ndarray, interface_commitment: Dict[Edge, float], xp, args: argparse.Namespace):
    if not candidates:
        return []
...
def candidate_edge_ups(interface_commitment: Dict[Edge, float], births: List[Dict[str, Any]], max_cands: int) -> List[Edge]:
    seen: List[Edge] = []
    for row in births:
        a, b = int(row["parents"][0]), int(row["parents"][1])
        child = int(row["child"])
...
def run_sim(pcfg: PhysicsConfig, scfg: bk.ScoreConfig, args: argparse.Namespace):
    ...
    for step in range(1, args.total_steps + 1):
        ...
        raw_births = generate_birth_candidates(psi_b, sigma, interface_commitment, active_nodes_b, args, xp)
        ranked_births = batched_birth_proxy(raw_births, sigma, interface_commitment, xp, args)
        births = ranked_births[: int(args.birth_topk_finalists)]
        ...
        ent_rows = entanglement_sources(psi, sigma, set(vis_nodes), xp, args) if vis_nodes else []
        snapshots.append({
            "step": int(step),
            ...
            "birth_finalist_count": int(len(births)),
        ...
        if int(args.progress_every) > 0 and step % int(args.progress_every) == 0:
            print(
                ...
                f"deltaF={(accepted['deltaF'] if accepted else None)} raw_births={len(raw_births)} finalists={len(births)} cand={len(diags)}"
            )

    return {
        "script": "hsf_mesoscape_entanglement_batched_birth.py",
        ...
        "run_config": {
            ...
            "birth_topk_finalists": int(args.birth_topk_finalists),
            ...
            "principle": "birth candidates are generated only from entangled parent pairs; all raw birth candidates are ranked by a cheap batched proxy, then only top finalists get exact witness scoring",
        },
