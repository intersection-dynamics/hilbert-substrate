...
def bandwidth_pressure(active_nodes, active_edges, core):
    ...
    # explicit support per organizer mass + edge overhead
...
def no_refolding_pressure(active_nodes, active_edges, core):
    ...
    # committed support complexity relative to organizer size
...
def no_forgetting_pressure(psi, core, n_sites, xp):
    ...
    rho = partial_trace_keep(psi, org, n_sites, xp)
    return float(von_neumann_entropy(rho, xp))
...
def birth_branch_compare(
    ...
    return {
        ...
        "suppressed": p_sup,
        "allowed": p_b,
        "delta_allowed_minus_suppressed": {
            k: float(p_b[k] - p_sup[k]) for k in p_sup.keys()
        },
        "organizer_fidelity_between_branches": float(organizer_fidelity),
        "interpretation_hint": {
            "bandwidth_relief_if_negative": float(p_b["bandwidth_pressure"] - p_sup["bandwidth_pressure"]),
            "no_forgetting_relief_if_negative": float(p_b["no_forgetting_pressure"] - p_sup["no_forgetting_pressure"]),
        },
    }
...
def reabsorption_branch_compare(
    ...
    # demote branch (support demotion, not factor deletion)
    ...
    return {
        ...
        "kept": p_keep,
        "demoted": p_demote,
        "delta_demoted_minus_kept": {
            k: float(p_demote[k] - p_keep[k]) for k in p_keep.keys()
        },
        "organizer_fidelity_between_branches": float(organizer_fidelity),
