# filename: hsf_factorization_native_sweep.py
#!/usr/bin/env python3
...
def main() -> None:
    p = argparse.ArgumentParser(
        description=(
            "Robustness sweep for hsf_factorization_native_observer.py. "
    ...
    p.add_argument("--outdir", type=str, default="hsf_factorization_native_sweep_out")
    args = p.parse_args()
    ...
    for seed, eps, ps in itertools.product(seeds, perturb_epses, pair_scales):
        row = run_one(
            ...
            total_dimension=args.total_dimension,
    ...
    for row in rows:
        ...
        if rf:
            runner_up_counts[str(rf)] = runner_up_counts.get(str(rf), 0) + 1

    json_path = outdir / "summary.json"
    with open(json_path, "w", encoding="utf-8") as f:
        json.dump(
            {
                "script": "hsf_factorization_native_sweep.py",
