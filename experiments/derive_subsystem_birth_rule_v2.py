...
def candidate_features(psi: np.ndarray, rho: np.ndarray, active_nodes: List[int], active_edges: List[Tuple[int, int]], edge_strengths: Dict[Tuple[int, int], float], dims: List[int], cache: Dict[Tuple[int, int], csr_matrix]) -> List[Dict[str, object]]:
    ...
    for i, j in combinations(active_nodes, 2):
        ...
        rows.append({
            "pair": [i, j], "mi": float(mi), "corr": float(corr), "sa": float(sa), "sb": float(sb),
            "pair_entropy": float(pair_entropy), "coord_gap": float(coord_gap), "common_nbr_count": int(len(common_nbrs)),
            "cmi_mean": float(cmi_mean), "cmi_max": float(cmi_max), "daughter_count": int(daughter_count), "score": score
        })
...
def spawn_births(chosen: List[Tuple[Dict[str, object], int]], active_nodes: List[int], dormant_nodes: List[int], active_edges: List[Tuple[int, int]], edge_strengths: Dict[Tuple[int, int], float], local_coeffs: Dict[int, np.ndarray], spawn_pair_scale: float, rng: np.random.Generator) -> List[Dict[str, object]]:
    ...
    for row, new_node in chosen:
        ...
        active_nodes.append(new_node); active_nodes.sort()
        e1 = (min(i, new_node), max(i, new_node))
        e2 = (min(j, new_node), max(j, new_node))
        if e1 not in active_edges: active_edges.append(e1)
        if e2 not in active_edges: active_edges.append(e2)
        active_edges.sort()
...
def maybe_decay_birth_links(psi: np.ndarray, rho: np.ndarray, active_edges: List[Tuple[int, int]], edge_strengths: Dict[Tuple[int, int], float], dims: List[int], cache: Dict[Tuple[int, int], csr_matrix], parent: int, new_node: int, mi_threshold: float, corr_threshold: float) -> Dict[str, float]:
    e = (min(parent, new_node), max(parent, new_node))
    if e not in active_edges:
        return {"mi": 0.0, "corr": 0.0, "alive": 0.0}
    rho_ab = partial_trace_keep(rho, dims, [parent, new_node])
    rho_a = partial_trace_keep(rho, dims, [parent])
    rho_b = partial_trace_keep(rho, dims, [new_node])
    mi = mutual_information(rho_ab, rho_a, rho_b)
    corr = pair_su3_correlator_strength(psi, cache, parent, new_node)
    alive = 1.0
    if mi < mi_threshold or corr < corr_threshold:
        active_edges.remove(e)
        edge_strengths.pop(e, None)
        alive = 0.0
    return {"mi": float(mi), "corr": float(corr), "alive": alive}

def classify_persistence(events: List[Dict[str, object]], active_nodes: List[int], active_edges: List[Tuple[int, int]], edge_strengths: Dict[Tuple[int, int], float], local_coeffs: Dict[int, np.ndarray], psi: np.ndarray, dims: List[int], cache: Dict[Tuple[int, int], csr_matrix], decay_mi_threshold: float, decay_corr_threshold: float, persist_windows_required: int, persist_entropy_threshold: float, persist_mean_mi_threshold: float, persist_triangle_threshold: int) -> List[Dict[str, object]]:
    triangles = active_triangles(active_nodes, active_edges)
    out = []
    for evt in events:
        i, j = evt["parents"]; n = evt["new_node"]
        rho = pure_density(psi)
        surv_i = maybe_decay_birth_links(psi, rho, active_edges, edge_strengths, dims, cache, i, n, decay_mi_threshold, decay_corr_threshold)
        surv_j = maybe_decay_birth_links(psi, rho, active_edges, edge_strengths, dims, cache, j, n, decay_mi_threshold, decay_corr_threshold)
        if n not in active_nodes:
            continue
        rho_n = partial_trace_keep(rho, dims, [n])
        sn = von_neumann_entropy(rho_n)
        links_alive = int(surv_i["alive"] + surv_j["alive"])
        mean_birth_mi = float(np.mean([surv_i["mi"], surv_j["mi"]]))
        tri_count = sum(1 for tri in triangles if n in tri and i in tri and j in tri)
        persistence_windows = 1 if links_alive == 2 else 0
        persistence_score = float(
            0.35 * persistence_windows +
            0.25 * (1.0 if sn >= persist_entropy_threshold else 0.0) +
            0.25 * (1.0 if mean_birth_mi >= persist_mean_mi_threshold else 0.0) +
            0.15 * (1.0 if tri_count >= persist_triangle_threshold else 0.0)
        )
        label = "persistent" if (
            persistence_windows >= persist_windows_required and
            sn >= persist_entropy_threshold and
            mean_birth_mi >= persist_mean_mi_threshold
        ) else "remerge_prone"
        if links_alive == 0 and n in active_nodes:
            ...
            label = "remerge_prone"
        out.append({
            "parents": [i, j], "new_node": n, "links_alive": links_alive, "new_node_entropy": float(sn),
            "mean_birth_mi": mean_birth_mi, "triangle_count_with_parents": int(tri_count),
            "persistence_windows": int(persistence_windows), "persistence_score": persistence_score, "label": label
        })
    return out
...
def run_sim(cfg: SimConfig) -> Dict[str, object]:
    ...
    for step in range(cfg.total_steps):
        ...
        if ((step + 1) % cfg.eval_every) == 0:
            ...
            spawn_events = spawn_births(chosen, active_nodes, dormant_nodes, active_edges, edge_strengths, local_coeffs, cfg.spawn_pair_scale, rng)
            for _ in range(cfg.lookahead_windows):
                ...
                psi = normalize_state(np.asarray(psi))
            labeled_births = classify_persistence(
                spawn_events, active_nodes, active_edges, edge_strengths, local_coeffs, psi, dims, cache,
                cfg.decay_mi_threshold, cfg.decay_corr_threshold, cfg.persist_windows_required,
                cfg.persist_entropy_threshold, cfg.persist_mean_mi_threshold, cfg.persist_triangle_threshold
            )
    ...
    X = []; y = []
    feature_names = ["mi", "corr", "sa", "sb", "pair_entropy", "coord_gap", "common_nbr_count", "cmi_mean", "cmi_max", "daughter_count", "score"]
    ...
    if len(set(y)) >= 2 and len(y) >= 8:
        ...
        derived_rule = {
            ...
            "n_labeled_births": len(y), "n_persistent": int(sum(y)), "n_remerge_prone": int(len(y) - sum(y))
        }
    else:
        derived_rule = {
            "feature_names": feature_names, "intercept": None, "coefficients": {},
            "n_labeled_births": len(y), "n_persistent": int(sum(y)) if y else 0, "n_remerge_prone": int(len(y) - sum(y)) if y else 0,
            "note": "Insufficient class diversity to fit logistic rule robustly."
        }
    summary = {
        "n_evals": total_evals, "n_candidates": len(candidate_dataset), "n_birth_events": len(birth_events),
        ...
        "active_nodes_final": len(active_nodes), "active_edges_final": len(active_edges)
    }
    return {
        "config": cfg.__dict__, "candidate_dataset": candidate_dataset, "birth_events": birth_events,
        "derived_rule": derived_rule, "summary": summary, "active_nodes_final": active_nodes,
        "active_edges_final": [list(e) for e in active_edges]
    }

def pretty_report(result: Dict[str, object]) -> str:
    cfg = result["config"]; summ = result["summary"]; rule = result["derived_rule"]
    lines = []
    lines.append("=" * 112)
    lines.append("DERIVE SUBSYSTEM BIRTH RULE (v2)")
    lines.append("-" * 112)
    lines.append(f"n_max={cfg['n_max']}  n_init={cfg['n_init']}  seed={cfg['seed']}  total_steps={cfg['total_steps']}  dt={cfg['dt']}  eval_every={cfg['eval_every']}  lookahead_windows={cfg['lookahead_windows']}")
    lines.append(f"candidate_fraction={cfg['candidate_fraction']}  fission_fraction={cfg['fission_fraction']}  birth_score_floor={cfg['birth_score_floor']}")
    lines.append(f"decay_mi_threshold={cfg['decay_mi_threshold']}  decay_corr_threshold={cfg['decay_corr_threshold']}  persist_windows_required={cfg['persist_windows_required']}")
    lines.append(f"persist_entropy_threshold={cfg['persist_entropy_threshold']}  persist_mean_mi_threshold={cfg['persist_mean_mi_threshold']}  persist_triangle_threshold={cfg['persist_triangle_threshold']}")
    lines.append("-" * 112)
    lines.append(f"Counts: evals={summ['n_evals']}  candidates={summ['n_candidates']}  births={summ['n_birth_events']}  persistent={summ['n_persistent_births']}  remerge_prone={summ['n_remerge_prone_births']}  final_nodes={summ['active_nodes_final']}  final_edges={summ['active_edges_final']}")
    lines.append("-" * 112)
    ...
    for evt in result["birth_events"][:12]:
        lines.append(f"  parents={evt['parents']}  new_node={evt['new_node']}  label={evt['label']}  links_alive={evt['links_alive']}  new_entropy={evt['new_node_entropy']:.4f}  mean_birth_mi={evt['mean_birth_mi']:.4f}  tri_with_parents={evt['triangle_count_with_parents']}  persist_score={evt['persistence_score']:.4f}")
    return "\\n".join(lines)

def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(description="Infer a better empirical subsystem birth/death rule from observed churn.")
    ...
    ap.add_argument("--candidate-fraction", type=float, default=0.35)
    ap.add_argument("--fission-fraction", type=float, default=0.30)
    ap.add_argument("--birth-score-floor", type=float, default=0.02)
    ap.add_argument("--decay-mi-threshold", type=float, default=0.06)
    ap.add_argument("--decay-corr-threshold", type=float, default=0.08)
    ap.add_argument("--persist-windows-required", type=int, default=1)
    ap.add_argument("--persist-entropy-threshold", type=float, default=0.08)
    ap.add_argument("--persist-mean-mi-threshold", type=float, default=0.08)
    ap.add_argument("--persist-triangle-threshold", type=int, default=0)
...
def main() -> int:
    args = parse_args()
    cfg = SimConfig(
        n_max=int(args.n_max), n_init=int(args.n_init), seed=int(args.seed),
        local_scale=float(args.local_scale), pair_scale=float(args.pair_scale), spawn_pair_scale=float(args.spawn_pair_scale),
        total_steps=int(args.total_steps), dt=float(args.dt), eval_every=int(args.eval_every), lookahead_windows=int(args.lookahead_windows),
        fission_fraction=float(args.fission_fraction), candidate_fraction=float(args.candidate_fraction), birth_score_floor=float(args.birth_score_floor),
        decay_mi_threshold=float(args.decay_mi_threshold), decay_corr_threshold=float(args.decay_corr_threshold),
        persist_windows_required=int(args.persist_windows_required), persist_entropy_threshold=float(args.persist_entropy_threshold),
        persist_mean_mi_threshold=float(args.persist_mean_mi_threshold), persist_triangle_threshold=int(args.persist_triangle_threshold),
