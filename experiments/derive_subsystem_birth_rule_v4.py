...
def evolve_windows(psi: np.ndarray, n_windows: int, dt: float, n_max: int, active_nodes: List[int], active_edges: List[Tuple[int, int]], cache: Dict[Tuple[int, int], csr_matrix], local_coeffs: Dict[int, np.ndarray], edge_strengths: Dict[Tuple[int, int], float]) -> np.ndarray:
    for _ in range(n_windows):
        H = build_hamiltonian(n_max, active_nodes, active_edges, cache, local_coeffs, edge_strengths)
        psi = expm_multiply((-1j * dt) * H, psi)
        psi = normalize_state(np.asarray(psi))
    return psi


def classify_births_multiwindow(events: List[Dict[str, object]], active_nodes: List[int], active_edges: List[Tuple[int, int]], edge_strengths: Dict[Tuple[int, int], float], local_coeffs: Dict[int, np.ndarray], psi: np.ndarray, dims: List[int], cache: Dict[Tuple[int, int], csr_matrix], cfg: SimConfig) -> Tuple[List[Dict[str, object]], np.ndarray]:
    out = []
    psi_work = psi.copy()
    ...
    for evt in events:
        ...
        for w in range(cfg.lookahead_windows):
            psi_work = evolve_windows(psi_work, 1, cfg.dt, cfg.n_max, active_nodes, active_edges, cache, local_coeffs, edge_strengths)
            rho = pure_density(psi_work)
            ...
            def link_stats(parent: int) -> Tuple[float, float, int]:
                ...
                mi = mutual_information(rho_ab, rho_a, rho_b)
                corr = pair_su3_correlator_strength(psi_work, cache, parent, n)
                alive = 1
                if w >= cfg.settling_windows:
                    if mi < cfg.decay_mi_threshold and corr < cfg.decay_corr_threshold:
            ...
            rho_n = partial_trace_keep(rho, dims, [n]) if n in active_nodes else np.array([[1.0]], dtype=complex)
            sn = von_neumann_entropy(rho_n) if n in active_nodes else 0.0
            mean_birth_mi = float(np.mean([mi_i, mi_j]))

            existing_edges = set((min(a, b), max(a, b)) for (a, b) in active_edges)
        ...
        full_windows = len(links_alive_hist)
        ...
        label = "persistent" if (
            ...
            pair_su3_correlator_strength(psi_work, cache, i, n) >= cfg.corr_survival_floor and
            pair_su3_correlator_strength(psi_work, cache, j, n) >= cfg.corr_survival_floor and
...
def run_sim(cfg: SimConfig) -> Dict[str, object]:
    ...
    while step < cfg.total_steps:
        ...
        if (step % cfg.eval_every) == 0:
            ...
            psi = evolve_windows(psi, cfg.settling_windows, cfg.dt, cfg.n_max, active_nodes, active_edges, cache, local_coeffs, edge_strengths)
            labeled, psi = classify_births_multiwindow(spawned, active_nodes, active_edges, edge_strengths, local_coeffs, psi, dims, cache, cfg)
...
def pretty_report(result: Dict[str, object]) -> str:
    ...
    lines = []
    lines.append("=" * 112)
    lines.append("DERIVE SUBSYSTEM BIRTH RULE (v4)")
    lines.append("-" * 112)
    lines.append(
    ...
    lines.append(
        ...
        f"persist_windows_required={cfg['persist_windows_required']}"
    )
    lines.append(
...
def main() -> int:
    ...
    cfg = SimConfig(
        ...
        mi_survival_floor=float(args.mi_survival_floor),
        corr_survival_floor=float(args.corr_survival_floor),
