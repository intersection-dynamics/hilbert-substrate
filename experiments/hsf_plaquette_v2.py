#!/usr/bin/env python3
"""
HSF Plaquette Spectrum — Production Sparse v2
================================================
Fix: Memory-bounded null-space finder for vertex 0.
Strategy: Process null-space in blocks that fit in RAM.

For 4v8l (Cartan=50625):
  - Vertex 0 null space ≈ 6750 (estimated from 3v6l scaling ratio)
  - Use block randomized SVD with 8000 probes → 50625×8000 = 6.5 GB ✓
  - After vertex 0: 6750 → ~900 → ~120 → ~16 (each vertex ~7.5x reduction)

Usage: python hsf_plaquette_v2.py [lattice_names...]
"""
...
def find_nullspace_large(Cv_sparse, n, mem_budget_gb=20.0):
    """
    Memory-bounded randomized null-space finder for large sparse PSD matrices.

    Algorithm:
      1. Estimate null-space dimension from small eigsh probe
      2. Use polynomial filter with k probes capped by memory budget
      3. QR + verify
    """
    bytes_per_complex = 16
    max_probe_cols = int(mem_budget_gb * 1e9 / (n * bytes_per_complex))
    max_probe_cols = min(max_probe_cols, n)
    print(f"    Memory budget {mem_budget_gb} GB → max {max_probe_cols} probe vectors", flush=True)

    # Estimate spectral gap
    n_probe = min(50, n - 2)
    try:
        ev_small = eigsh(Cv_sparse, k=n_probe, which='SM', tol=1e-8, maxiter=3000)[0]
    except:
        ev_small = eigsh(Cv_sparse, k=n_probe, which='SM', tol=1e-6, maxiter=5000)[0]
    ev_small = np.sort(ev_small)
    n_zero_probe = int(np.sum(np.abs(ev_small) < 1e-6))
    nonzero = ev_small[ev_small > 1e-6]
    lambda_gap = nonzero[0] if len(nonzero) > 0 else 3.0

    print(f"    Probe: {n_zero_probe}/{n_probe} zeros, gap ≈ {lambda_gap:.4f}", flush=True)

    # Estimate null dimension: extrapolate from probe
    if n_zero_probe == n_probe:
        # All probed are zero. Use scaling from known lattices:
        # 3v6l: 3375 → 450 per vertex (ratio 0.133)
        # Estimate conservatively with 1.5x safety
        est_null = int(n * 0.2)
    else:
        est_null = int(n * n_zero_probe / n_probe * 2.0)
    est_null = max(est_null, n_zero_probe + 20)

    k = min(est_null + 100, max_probe_cols, n)
    print(f"    Estimated null dim ≈ {est_null}, using k={k} probes", flush=True)

    probe_mem_gb = n * k * bytes_per_complex / 1e9
    print(f"    Probe matrix: {n}×{k} = {probe_mem_gb:.1f} GB", flush=True)

    # Polynomial filter
    try:
        ev_large = eigsh(Cv_sparse, k=3, which='LM', tol=1e-6)[0]
        lambda_max = max(ev_large) * 1.05
    except:
        lambda_max = 30.0
    scale = lambda_max
    power = 40  # (1-3/scale)^40 is plenty of suppression

    ratio = 1.0 - lambda_gap / scale
    suppression = abs(ratio) ** power
    print(f"    Filter: scale={scale:.2f}, ratio={ratio:.4f}, power={power}, "
          f"suppression={suppression:.2e}", flush=True)

    I_sp = sparse.eye(n, format='csr')
    filter_op = I_sp - Cv_sparse / scale

    np.random.seed(42)
    Y = np.random.randn(n, k).astype(np.float64) + 1j * np.random.randn(n, k).astype(np.float64)

    for p in range(power):
        Y = filter_op @ Y
        if p % 10 == 9:
            Y, _ = np.linalg.qr(Y, mode='reduced')
            print(f"      power iter {p+1}/{power}", flush=True)

    Q, _ = np.linalg.qr(Y, mode='reduced')
    del Y; gc.collect()

    # Check which columns are in null space
    CQ = Cv_sparse @ Q
    col_norms = np.linalg.norm(CQ, axis=0)
    del CQ; gc.collect()

    null_mask = col_norms < 1e-6
    n_null = int(np.sum(null_mask))
    print(f"    Found {n_null} null vectors", flush=True)

    if n_null == 0:
        print("    WARNING: No null vectors found. Trying direct eigsh...", flush=True)
        ev, P = eigsh(Cv_sparse, k=min(200, n-1), which='SM', tol=1e-10, maxiter=10000)
        mask = np.abs(ev) < 1e-6
        return P[:, mask]

    P_null = Q[:, null_mask].copy()
    del Q; gc.collect()

    P_null, _ = np.linalg.qr(P_null, mode='reduced')
    residual = np.max(np.linalg.norm(Cv_sparse @ P_null, axis=0))
    print(f"    Null space: dim={P_null.shape[1]}, residual={residual:.2e}", flush=True)

    # Verify we got them all: check if there are more
    # Project Cv to complement of found null space and check for more zeros
    if n_null >= k - 50:
        print(f"    WARNING: null space may be truncated (found {n_null} of {k} probes).")
        print(f"    Consider increasing memory budget.", flush=True)
    ...
    for v in range(n_vertices):
        ...
        if P is None:
            if n_cartan <= 10000:
                ...
                # Large sparse path
                P = find_nullspace_large(Cv, n_cartan, mem_budget_gb=mem_budget_gb)
        else:
...
# Main analysis
# ============================================================
def analyze(name, n_workers=None, mem_gb=20.0):
    ...
    n_singlet, P = find_singlet_sector(cartan_states, state_to_idx, n_vertices,
                                        outgoing, incoming, n_cartan, mem_budget_gb=mem_gb)
...
def main():
    print("=" * 70)
    print("HSF PLAQUETTE SPECTRUM — PRODUCTION v2")
    print("=" * 70)
    n_workers = min(cpu_count(), 12)
    print(f"CPUs: {cpu_count()}, workers: {n_workers}", flush=True)

    # Parse args. Accept optional --mem flag
    names = []
    mem_gb = 20.0
    args = sys.argv[1:]
    i = 0
    while i < len(args):
        if args[i] == '--mem':
            mem_gb = float(args[i+1]); i += 2
        else:
            names.append(args[i]); i += 1
    if not names:
        names = ["2v4l", "3v6l", "4v8l"]

    print(f"Memory budget: {mem_gb} GB", flush=True)

    t_total = time.time()
    results = []
    for name in names:
        try:
            r = analyze(name, n_workers, mem_gb)
    ...
    with open("hsf_plaquette_v2.json", "w") as f:
        json.dump(results, f, indent=2, default=str)
    print(f"\nSaved to hsf_plaquette_v2.json")
