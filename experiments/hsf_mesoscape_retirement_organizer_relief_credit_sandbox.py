#!/usr/bin/env python3
# filename: hsf_mesoscape_retirement_organizer_relief_credit_sandbox.py
...
def _edge_component_score(
    ...
    # bookkeeping-visible proxy only
...
def _build_edge_delta_rows(node: int, snap: Dict[str, Any]) -> Dict[str, Any]:
    active_edges = snap.get("active_edges") or []
    before_edge_set = set(
...
def _organizer_relief_credit(
    ...
    organizer_relief_weight: float,
    neighbor_relief_weight: float,
    incident_loss_weight: float,
    cap: float,
) -> Dict[str, float]:
    core_credit = 0.0
    neighbor_relief = 0.0
    ...
    for row in edge_deltas:
        ...
        if edge_class == "incident_lost" and delta < 0.0:
            incident_loss += abs(delta)

    gross = organizer_relief_weight * core_credit + neighbor_relief_weight * neighbor_relief
    net = gross - incident_loss_weight * incident_loss
    credit = max(0.0, min(cap, net))

    return {
        "organizer_core_relief_credit": float(core_credit),
        "neighbor_relief_credit": float(neighbor_relief),
        "incident_loss_burden": float(incident_loss),
...
def _rows_from_run(data: Dict[str, Any], args: argparse.Namespace) -> List[Dict[str, Any]]:
    ...
    for snap in data.get("snapshots", []) or []:
        ...
        edge_info = _build_edge_delta_rows(node, snap)
        credit_info = _organizer_relief_credit(
            core_pair=core_pair,
            edge_deltas=edge_info["edge_deltas"],
...
def _summary(rows: List[Dict[str, Any]]) -> Dict[str, Any]:
    return {
        ...
        "incident_loss_mean": _mean([r["incident_loss_burden"] for r in rows]),
    }
...
def _per_node_summary(rows: List[Dict[str, Any]], node_id: int) -> Dict[str, Any]:
    ...
    return {
        ...
        "incident_loss_mean": _mean([r["incident_loss_burden"] for r in group]),
    }
...
def _readout(late_rows: List[Dict[str, Any]], node_summaries: Dict[str, Any]) -> List[str]:
    ...
    if any(r["would_flip_positive"] for r in late_rows):
        reads.append("Organizer-relief credit can flip some late losing retirements to positive total score.")
    if any(r["would_beat_winner"] for r in late_rows):
        reads.append("Organizer-relief credit can make some late retirements beat the accepted winner.")
    if "2" in node_summaries and "6" in node_summaries:
        n2 = node_summaries["2"]
        n6 = node_summaries["6"]
        if n6["incident_loss_mean"] > n2["incident_loss_mean"]:
            reads.append("Node 6 carries a larger incident-edge loss burden than node 2.")
        if n6["core_relief_mean"] <= n2["core_relief_mean"]:
            reads.append("Node 6 does not get extra organizer-edge relief relative to node 2 under the current sandbox.")
        if n6["sandbox_lower_deltaF_mean"] > n6["lower_deltaF_mean"]:
            reads.append("Organizer-relief credit partially rescues node 6, even if not enough to reverse the sign.")
    if not reads:
        reads.append("Organizer-relief credit does not materially change the late retirement ranking under this sandbox.")
    return reads
...
def _flagged_rows(rows: List[Dict[str, Any]], limit: int = 20) -> List[Dict[str, Any]]:
    return sorted(
        rows,
        key=lambda r: (r["deltaF_improvement"], r["sandbox_lower_deltaF"]),
        reverse=True,
    )[:limit]
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Sandbox for retirement organizer-relief credit. "
            "Adds an organizer-edge / neighbor-survival relief credit to lower_support candidates "
            "and reports whether it would rescue late demotions."
        )
    )
    ...
    parser.add_argument(
        "--json-out",
        default=None,
        help="Optional output path. Default: <input>_retirement_organizer_relief_credit_sandbox.json",
    )
    ...
    report = {
        "script": "hsf_mesoscape_retirement_organizer_relief_credit_sandbox.py",
    ...
    out_path = Path(args.json_out) if args.json_out else in_path.with_name(in_path.stem + "_retirement_organizer_relief_credit_sandbox.json")
    with out_path.open("w", encoding="utf-8") as f:
        json.dump(report, f, indent=2)

    print("=== Retirement Organizer-Relief Credit Sandbox ===")
