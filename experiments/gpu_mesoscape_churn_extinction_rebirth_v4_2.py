...
def connected_components(active_nodes, active_edges):
...
def bridge_edges(active_nodes, active_edges):
    adj = {n: [] for n in active_nodes}
    edge_set = set()
    for a, b in active_edges:
        if a in adj and b in adj:
...
def spectral_1d_embedding(active_nodes, active_edges, link_states):
    ...
    comps = connected_components(active_nodes, active_edges)
    if not comps:
        return {}
    coords = {}
    offset = 0.0
    for comp in sorted(comps, key=len, reverse=True):
        if len(comp) == 1:
            coords[comp[0]] = offset
            offset += 3.0
            continue
        comp_edges = [e for e in active_edges if e[0] in comp and e[1] in comp]
        W, idx_of = weighted_adjacency(comp, comp_edges, link_states)
        deg = np.sum(W, axis=1)
        if np.allclose(deg, 0.0):
            xs = np.arange(len(comp), dtype=float)
        else:
            L = np.diag(deg) - W
            vals, vecs = np.linalg.eigh(L)
            xs = np.real(vecs[:, 1]) if len(vals) >= 2 else np.arange(len(comp), dtype=float)
            xs = xs - np.mean(xs)
            s = np.std(xs)
            if s > 1e-12:
                xs = xs / s
        for node in comp:
            coords[node] = float(xs[idx_of[node]] + offset)
        span = float(np.max(xs) - np.min(xs)) if len(xs) else 0.0
        offset += max(3.0, span + 3.0)
    return coords
...
def fidelity_uhlmann(rho, sigma):
    ...
    fid = float(np.real(np.trace(smid)) ** 2)
...
class SimConfig:
    ...
    bootstrap_mi_floor: float
    bootstrap_corr_floor: float
    bootstrap_birth_score_floor: float
    ...
    triangle_edge_protection: float
    bridge_edge_protection: float
    core_edge_protection: float
    edge_bandwidth_prune_thresh: float
    edge_commitment_prune_thresh: float
    edge_prune_grace_evals: int
    max_edge_prunes_per_eval: int
    ...
    persist_score_threshold: float
    persist_alive_fraction_floor: float
    orphan_basis0_floor: float
    reabs_edge_margin: int
    min_post_reabs_structure_score: int
    progress_every: int
    device: str
    json_out: str
...
def copy_link_states(link_states):
    out = {}
    for e, d in link_states.items():
        out[e] = {
            k: (v.copy() if hasattr(v, "copy") else list(v) if isinstance(v, list) else v)
            for k, v in d.items()
        }
    return out


def constrained_expression_metrics(psi, active_nodes, active_edges, link_states, GM, GMbar, cfg, xp):
    core = dominant_core_snapshot(psi, active_nodes, active_edges, link_states, GM, GMbar, xp, cfg.n_max)
    shell_size = int(core["shell_size"]) if core else 0
    tri_count = len(active_triangles(active_nodes, active_edges))
    coords = spectral_1d_embedding(active_nodes, active_edges, link_states)
    signal = 0.0
    if core and coords:
        cp = core["core_pair"]
        c0 = 0.5 * (coords.get(cp[0], 0.0) + coords.get(cp[1], 0.0))
        shell = organizer_nodes_from_core(core)
        if shell:
            signal = float(np.mean([abs(coords.get(n, c0) - c0) for n in shell]))
    bw_vals = []
    pur_vals = []
    rank_vals = []
    mi_vals = []
    for i, j in active_edges:
        ...
        if e not in link_states:
            continue
        ls = link_states[e]
        bw_vals.append(float(ls.get("bandwidth_score", 0.0)))
        pur_vals.append(float(ls.get("purity", 0.0)))
        rank_vals.append(float(ls.get("bandwidth_rank", 0)))
        try:
            mi_vals.append(mutual_information_from_state(psi, i, j, cfg.n_max, xp))
        except Exception:
            pass
    mean_bw = float(np.mean(bw_vals)) if bw_vals else 0.0
    mean_pur = float(np.mean(pur_vals)) if pur_vals else 0.0
    mean_rank = float(np.mean(rank_vals)) if rank_vals else 0.0
    mean_mi = float(np.mean(mi_vals)) if mi_vals else 0.0
    bandwidth_pressure = float((len(active_nodes) + 0.5 * len(active_edges)) / max(1, shell_size))
    refolding_pressure = float((len(active_edges) + tri_count) / max(1, shell_size))
    signaling_pressure = float(signal)
    support_integrity = float(1.2 * mean_bw + 0.5 * mean_pur + 0.25 * mean_rank + 0.35 * mean_mi + 0.10 * shell_size)
    total_burden = float(bandwidth_pressure + 0.7 * refolding_pressure + 0.4 * signaling_pressure)
    net_score = float(support_integrity - total_burden)
    return {
        "core": core,
        "shell_size": shell_size,
        "triangle_count": tri_count,
        "mean_link_bw": mean_bw,
        "mean_link_purity": mean_pur,
        "mean_link_rank": mean_rank,
        "mean_edge_mi": mean_mi,
        "bandwidth_pressure": bandwidth_pressure,
        "refolding_pressure": refolding_pressure,
        "signaling_pressure": signaling_pressure,
...
def local_expression_metrics(psi, active_nodes, active_edges, link_states, parents, GM, GMbar, cfg, xp):
    i, j = parents
    focal_nodes = set([i, j])
    for a, b in active_edges:
    ...
    focal_edges = []
    for a, b in active_edges:
        if a in focal_nodes and b in focal_nodes:
            focal_edges.append((a, b))
    tri_count = 0
    for tri in combinations(sorted(focal_nodes), 3):
        a, b, c = tri
        es = {(min(a,b),max(a,b)), (min(a,c),max(a,c)), (min(b,c),max(b,c))}
        if all(e in set((min(x,y),max(x,y)) for x,y in focal_edges) for e in es):
            tri_count += 1
    mi_sum = 0.0
    mi_count = 0
    bw_sum = 0.0
    ...
    rank_sum = 0.0
    link_count = 0
    for a, b in focal_edges:
        try:
            mi_sum += mutual_information_from_state(psi, a, b, cfg.n_max, xp)
            mi_count += 1
        except Exception:
            pass
        e = (min(a, b), max(a, b))
        ls = link_states.get(e)
        if ls is None:
            continue
        bw_sum += float(ls.get('bandwidth_score', 0.0))
        pur_sum += float(ls.get('purity', 0.0))
        rank_sum += float(ls.get('bandwidth_rank', 0.0))
        link_count += 1
    try:
        parent_mi = mutual_information_from_state(psi, i, j, cfg.n_max, xp)
    except Exception:
        parent_mi = 0.0
    try:
        parent_corr = pair_su3_correlator_strength(psi, GM, GMbar, i, j, xp)
    except Exception:
        parent_corr = 0.0
    return {
        'local_edge_mi_sum': float(mi_sum),
        'mean_local_edge_mi': float(mi_sum / mi_count) if mi_count else 0.0,
        'mean_local_bw': float(bw_sum / link_count) if link_count else 0.0,
        'mean_local_purity': float(pur_sum / link_count) if link_count else 0.0,
        'mean_local_rank': float(rank_sum / link_count) if link_count else 0.0,
        'local_triangle_count': int(tri_count),
        'local_shell_size': int(len(focal_nodes)),
        'parent_interface_mi': float(parent_mi),
        'parent_interface_corr': float(parent_corr),
    }

def birth_counterfactual_gain(row, new_node, psi, active_nodes, dormant_nodes, active_edges, link_states, base_local_coeffs, GM, GMbar, cfg, xp):
    windows = max(1, cfg.lookahead_windows)
    parents = tuple(row["pair"])

    psi_no = psi.copy()
    nodes_no = list(active_nodes)
    dormant_no = list(dormant_nodes)
    edges_no = list(active_edges)
    links_no = copy_link_states(link_states)
    coeffs_no = {k: np.array(v, copy=True) for k, v in base_local_coeffs.items()}
    sanitize_graph_state(nodes_no, dormant_no, edges_no, links_no)
    update_all_links(psi_no, edges_no, links_no, GM, GMbar, cfg, xp)
    psi_no = evolve_windows(psi_no, windows, cfg, nodes_no, edges_no, coeffs_no, links_no, GM, GMbar, xp)
    update_all_links(psi_no, edges_no, links_no, GM, GMbar, cfg, xp)
    met_no = constrained_expression_metrics(psi_no, nodes_no, edges_no, links_no, GM, GMbar, cfg, xp)
    local_no = local_expression_metrics(psi_no, nodes_no, edges_no, links_no, parents, GM, GMbar, cfg, xp)

    psi_yes = psi.copy()
    nodes_yes = list(active_nodes)
    dormant_yes = list(dormant_nodes)
    edges_yes = list(active_edges)
    links_yes = copy_link_states(link_states)
    coeffs_yes = {k: np.array(v, copy=True) for k, v in base_local_coeffs.items()}
    sanitize_graph_state(nodes_yes, dormant_yes, edges_yes, links_yes)
    _ = spawn_births([(row, new_node)], psi_yes, nodes_yes, dormant_yes, edges_yes, links_yes, coeffs_yes, GM, GMbar, cfg, np.random.default_rng(cfg.seed + 1000 + int(new_node)), -1, xp)
    update_all_links(psi_yes, edges_yes, links_yes, GM, GMbar, cfg, xp)
    psi_yes = evolve_windows(psi_yes, windows, cfg, nodes_yes, edges_yes, coeffs_yes, links_yes, GM, GMbar, xp)
    update_all_links(psi_yes, edges_yes, links_yes, GM, GMbar, cfg, xp)
    met_yes = constrained_expression_metrics(psi_yes, nodes_yes, edges_yes, links_yes, GM, GMbar, cfg, xp)
    local_yes = local_expression_metrics(psi_yes, nodes_yes, edges_yes, links_yes, parents, GM, GMbar, cfg, xp)

    integrity_gain = float(met_yes["support_integrity"] - met_no["support_integrity"])
    burden_relief = float(met_no["total_burden"] - met_yes["total_burden"])
    burden_penalty = float(max(0.0, met_yes["total_burden"] - met_no["total_burden"]))

    expression_gain = float(
        1.40 * (local_yes["local_edge_mi_sum"] - local_no["local_edge_mi_sum"])
        + 1.25 * (local_yes["parent_interface_mi"] - local_no["parent_interface_mi"])
        + 0.55 * (local_yes["parent_interface_corr"] - local_no["parent_interface_corr"])
        + 0.80 * (local_yes["mean_local_bw"] - local_no["mean_local_bw"])
        + 0.35 * (local_yes["mean_local_rank"] - local_no["mean_local_rank"])
        + 0.25 * (local_yes["mean_local_purity"] - local_no["mean_local_purity"])
        + 0.30 * (local_yes["local_triangle_count"] - local_no["local_triangle_count"])
        + 0.10 * (local_yes["local_shell_size"] - local_no["local_shell_size"])
    )
    unresolved_load = float(
        0.55 * max(row.get("mi", 0.0), 0.0)
        + 0.30 * max(row.get("corr", 0.0), 0.0)
        + 0.10 * max(row.get("pair_bandwidth_score", 0.0), 0.0)
        + 0.05 * max(row.get("cmi_mean", 0.0), 0.0)
    )
    witness = float(expression_gain + 0.35 * integrity_gain + 0.20 * burden_relief - 0.30 * burden_penalty + unresolved_load)
    return witness, {
        "no_birth": {k: v for k, v in met_no.items() if k != "core"},
        "with_birth": {k: v for k, v in met_yes.items() if k != "core"},
        "local_no_birth": local_no,
        "local_with_birth": local_yes,
        "expression_gain": expression_gain,
        "unresolved_load": unresolved_load,
...
def choose_candidate_births(rows, dormant_nodes, psi, active_nodes, active_edges, link_states, base_local_coeffs, GM, GMbar, cfg, xp):
    if not rows or not dormant_nodes:
        return []

    n_considered = max(1, int(np.ceil(cfg.candidate_fraction * len(rows))))
    considered = rows[: min(cfg.max_birth_candidates, n_considered)]

    chosen = []
    used_nodes = set()
    used_common = set()
    used_centers: List[float] = []
    dormant_pool = list(dormant_nodes)

    for row in considered:
        if len(chosen) >= min(cfg.max_births_per_eval, len(dormant_pool)):
            break
        i, j = row["pair"]
        ...
        if not dormant_pool:
            break
        new_node = dormant_pool[0]
        witness, diag = birth_counterfactual_gain(row, new_node, psi, active_nodes, dormant_nodes, active_edges, link_states, base_local_coeffs, GM, GMbar, cfg, xp)
        row["counterfactual_witness"] = float(witness)
        row["counterfactual_diag"] = diag
        if witness <= 0.0:
            continue
        dormant_pool.pop(0)
...
def classify_births_multiwindow(events, active_nodes, active_edges, link_states, base_local_coeffs, psi, GM, GMbar, cfg, xp):
    ...
    for evt in events:
        ...
        mi_hist, shell_hist, common_hist, entropy_hist, rank_hist, alive_hist = [], [], [], [], [], []
        for _ in range(cfg.lookahead_windows):
            ...
            if n not in active_nodes_w:
                break
            e1, e2 = (min(i, n), max(i, n)), (min(j, n), max(j, n))
            mi_i = mutual_information_from_state(psi_w, i, n, cfg.n_max, xp) if e1 in link_states_w else 0.0
            mi_j = mutual_information_from_state(psi_w, j, n, cfg.n_max, xp) if e2 in link_states_w else 0.0
        ...
        mean_mi = float(np.mean(mi_hist)) if mi_hist else 0.0
        mean_ent = float(np.mean(entropy_hist)) if entropy_hist else 0.0
        mean_common = float(np.mean(common_hist)) if common_hist else 0.0
        mean_shell = float(np.mean(shell_hist)) if shell_hist else 0.0
        mean_rank = float(np.mean(rank_hist)) if rank_hist else 0.0
        alive_frac = float(np.mean([1.0 if a >= 1 else 0.0 for a in alive_hist])) if alive_hist else 0.0
        persistence_score = mean_mi + 0.04 * mean_rank + 0.02 * mean_common + 0.03 * mean_shell + 0.03 * alive_frac
        label = "persistent" if persistence_score >= cfg.persist_score_threshold and alive_frac >= cfg.persist_alive_fraction_floor else "remerge_prone"
        out.append({
            "parents": [i, j],
            "new_node": int(n),
            "mean_birth_mi": mean_mi,
            "mean_new_node_entropy": mean_ent,
...
def live_edge_prune(active_nodes, active_edges, link_states, core, cfg, eval_idx=0):
    if not active_edges:
        return []
    ...
    for tri in active_triangles(active_nodes, active_edges):
        a,b,c = tri
        triangle_edges.update({(min(a,b),max(a,b)),(min(a,c),max(a,c)),(min(b,c),max(b,c))})
    bridges = bridge_edges(active_nodes, active_edges)
    core_pair = tuple(sorted(core.get("core_pair", []))) if core else None
    candidates = []
    for i, j in active_edges:
        e = (min(i, j), max(i, j))
        if e not in link_states:
        ...
        age = max(0, eval_idx - int(ls.get("birth_eval", 0)))
        protected = 0.0
        if e in shell_edges:
            protected += cfg.shell_edge_protection
        if e in triangle_edges:
            protected += cfg.triangle_edge_protection
        if e in bridges:
            protected += cfg.bridge_edge_protection
        if core_pair and e == core_pair:
            protected += cfg.core_edge_protection
        weak = (bw + protected < cfg.edge_bandwidth_prune_thresh) and (pur < cfg.edge_commitment_prune_thresh)
        if age < cfg.edge_prune_grace_evals:
            weak = False
        candidates.append((bw + pur + protected, weak, e, bw, pur, age))
    weak_edges = [x for x in candidates if x[1]]
    if not weak_edges:
        return []
    weak_edges.sort(key=lambda t: t[0])
    pruned = []
    to_remove = set()
    for _, _, e, bw, pur, age in weak_edges:
        if len(pruned) >= cfg.max_edge_prunes_per_eval:
            break
        remaining = len(active_edges) - len(to_remove) - 1
        if remaining < cfg.min_active_edges_floor:
            continue
        if e in bridges and remaining <= cfg.min_active_edges_floor + 1:
            continue
        to_remove.add(e)
        pruned.append({"edge": [e[0], e[1]], "bandwidth_score": bw, "purity": pur, "age": age})
    if to_remove:
        active_edges[:] = [e for e in active_edges if tuple(sorted(e)) not in to_remove]
        for e in to_remove:
            link_states.pop(e, None)
    return pruned


def cleanup_isolated_near_vacuum(psi, active_nodes, dormant_nodes, active_edges, link_states, core, cfg, xp):
    if not active_nodes:
        return []
...
def commit_reabsorptions(psi, active_nodes, dormant_nodes, active_edges, link_states, core, GM, GMbar, cfg, xp):
    ...
    if len(active_edges) <= cfg.min_active_edges_floor + cfg.reabs_edge_margin:
        return []
    candidates = []
    for n in active_nodes:
        if n in shell:
        ...
        if any(e in bridges for e in incident_edges):
            continue
        bw_relief = 0.0
        for e in incident_edges:
            if e in link_states:
                bw_relief += float(link_states[e].get("bandwidth_score", 0.0))
        candidates.append((basis0 + 0.15 * bw_relief - 0.08 * deg, n, basis0))
    ...
    for _, n, basis0 in candidates[: cfg.max_live_reabsorbs]:
        ...
        if len(active_edges) - len(incident) < cfg.min_active_edges_floor + cfg.reabs_edge_margin:
            continue
        ...
        if shell_size2 < cfg.min_shell_size_floor:
            continue
        gs2 = graph_stats(active_nodes2, active_edges2)
        if gs2["triangle_count"] + (1 if shell_size2 >= cfg.min_shell_size_floor else 0) < cfg.min_post_reabs_structure_score:
            continue
...
def parse_args():
    p = argparse.ArgumentParser(description="HSF mesoscape churn v4.2 cleanup pass with structural retention")
    ...
    p.add_argument("--bootstrap-mi-floor", type=float, default=0.02)
    p.add_argument("--bootstrap-corr-floor", type=float, default=0.02)
    p.add_argument("--bootstrap-birth-score-floor", type=float, default=0.002)
    ...
    p.add_argument("--triangle-edge-protection", type=float, default=1.0)
    p.add_argument("--bridge-edge-protection", type=float, default=1.5)
    p.add_argument("--core-edge-protection", type=float, default=1.5)
    p.add_argument("--edge-bandwidth-prune-thresh", type=float, default=0.18)
    p.add_argument("--edge-commitment-prune-thresh", type=float, default=0.10)
    p.add_argument("--edge-prune-grace-evals", type=int, default=3)
    p.add_argument("--max-edge-prunes-per-eval", type=int, default=1)
    ...
    p.add_argument("--persist-score-threshold", type=float, default=0.12)
    p.add_argument("--persist-alive-fraction-floor", type=float, default=0.60)
    p.add_argument("--orphan-basis0-floor", type=float, default=0.92)
    p.add_argument("--reabs-edge-margin", type=int, default=1)
    p.add_argument("--min-post-reabs-structure-score", type=int, default=2)
    p.add_argument("--progress-every", type=int, default=1)
    p.add_argument("--device", type=str, default="cpu", choices=["cpu", "gpu"])
    p.add_argument("--json-out", type=str, default="gpu_mesoscape_churn_extinction_rebirth_v4.json")
    a = p.parse_args()
...
def run_sim(cfg: SimConfig):
    ...
    for eval_idx in range(1, n_evals + 1):
        ...
        core = dominant_core_snapshot(psi, active_nodes, active_edges, link_states, GM, GMbar, xp, cfg.n_max)
        pruned = live_edge_prune(active_nodes, active_edges, link_states, core, cfg, eval_idx)
        if pruned:
            edge_prunes.extend([{**p, "step": step} for p in pruned])
        sanitize_graph_state(active_nodes, dormant_nodes, active_edges, link_states)
        orphan_events = cleanup_isolated_near_vacuum(psi, active_nodes, dormant_nodes, active_edges, link_states, core, cfg, xp)
        if orphan_events:
            extinction_events.extend([{**e, "step": step} for e in orphan_events])
        rows = candidate_features(psi, active_nodes, active_edges, link_states, cfg.n_max, GM, GMbar, cfg, xp)
        chosen = choose_candidate_births(rows, dormant_nodes, psi, active_nodes, active_edges, link_states, base_local_coeffs, GM, GMbar, cfg, xp)
