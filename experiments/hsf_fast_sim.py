#!/usr/bin/env python3
"""
HSF TURBO: C++ Accelerated Simulation
=====================================
Writes, compiles, and runs a C++ engine for instant HSF physics.
Requires a C++ compiler (cl.exe, g++, or clang++) in your PATH.
"""
...
def detect_compiler():
    ...
    try:
        ...
        # cl returns non-zero when no args, that's fine, it exists
        return "msvc"
...
def main():
    ...
    print(f"[HSF TURBO] N={args.N} Seeds={args.seeds} Steps={args.steps}")
    
    # 1. Write CPP
    cpp_path = out_dir / "hsf_engine.cpp"
    exe_path = out_dir / "hsf_engine"
    ...
    if not compiler:
        print("ERROR: No C++ compiler found (cl.exe or g++). Cannot run TURBO mode.")
        print("Please install Visual Studio Build Tools or MinGW.")
        sys.exit(1)
        
    print(f"[Compiling] Using {compiler}...")
    t0 = time.time()
    
    if compiler == "msvc":
        # /O2 optimization, /openmp, /EHsc (exceptions)
        cmd = ["cl", "/O2", "/openmp", "/EHsc", str(cpp_path), f"/Fe{str(exe_path)}"]
        # Suppress output
        subprocess.run(cmd, check=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
        
    elif compiler == "g++":
        # -O3, -fopenmp
        cmd = ["g++", "-O3", "-fopenmp", str(cpp_path), "-o", str(exe_path)]
    ...
    # Run exe and capture stdout lines
    cmd = [str(exe_path), str(args.N), str(args.seeds), str(args.steps)]
    result = subprocess.run(cmd, capture_output=True, text=True)
    
    if result.returncode != 0:
        print("Error running engine:")
        print(result.stderr)
        sys.exit(1)
    ...
    dt = time.time() - t0
    print(f"[Finished] {len(rows)} seeds in {dt:.2f}s")
    
    # 4. Save
    jsonl_path = out_dir / "runs" / "runs.jsonl"
    ...
    # Report
    if rows:
        v_after = [r["v2_v1_after"] for r in rows]
        mean_v = sum(v_after)/len(v_after)
        print(f"Mean V2/V1 After: {mean_v:.4f}")
        
        with open(out_dir / "REPORT.md", "w") as f:
            f.write(f"# HSF Turbo Report\nRuntime: {dt:.2f}s\nMean V2/V1: {mean_v:.4f}\n")
