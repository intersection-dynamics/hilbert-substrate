...
def method_perturbative(lattice, coupling=0.5):
    ...
    # Basis ordering is (sites ⊗ bonds) with flat index: idx = site_index*d_b + bond_index
    P_sites = np.outer(psi_sites, psi_sites.conj())  # (d_s × d_s)
    P = np.kron(P_sites, np.eye(d_b, dtype=complex)) # (D × D)
    Q = np.eye(D, dtype=complex) - P

    # Sanity check: projector idempotence
    proj_err = np.linalg.norm(P @ P - P)
    if proj_err > 1e-8:
        print(f"  [WARN] Projector sanity check ||P^2-P|| = {proj_err:.2e}")
    ...
    for i in range(lattice.n_sites):
        H_site += (-Delta) * lattice._embed_operator(sz, [i])  # ground is |0⟩ for each site
    
    # Now H = H_0 + V where H_0 = H_site, V = H_full (the coupling)
    # E_0 = ground state energy of H_0 in site sector = -N*Delta (all sites in |0⟩)
...
if __name__ == '__main__':
    ...
    out_path = os.path.join(out_dir, f"bond_hamiltonian_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png")
