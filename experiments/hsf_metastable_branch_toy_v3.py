# hsf_metastable_branch_toy_v3.py
# ------------------------------------------------------------
# HSF-inspired toy model: bias selection into a metastable branch (v3)
#
# v3 adds PHASE-SENSITIVE diagnostics (requested next clean move):
#   - All v2 features:
#       * cumulative + rolling branch fractions
#       * hide ticks where rolling captures < min_captures_plot
#       * print final delta, SE, z-score (cumulative)
#       * save clean plot
#   - PLUS phase-binned measurement vs drive phase:
#       * bins events by phi = (drive_w*t + drive_phase) mod 2π
#       * computes pM_in(phi), pM_out(phi), delta(phi)
#       * plots phase response (this is the key "is drive doing anything?" test)
#
# SAFE MODEL (generic branching, non-nuclear).
#
# Typical run (Windows one-liner):
#   python hsf_metastable_branch_toy_v3.py --N 250 --p_edge 0.03 --T 20000 --lambda_capture 0.09 --drive_frac 0.20 --drive_amp 0.35 --drive_w 0.03 --drive_gain 2.2 --phase_bins 24 --seed 0 --run_name phase_test
# ------------------------------------------------------------
...
class Params:
    ...
    # phase diagnostics
    phase_bins: int
    min_captures_phase: int  # hide bins with too few events
    ...
    for t in range(T):
        ...
        if E > 0:
            ...
            for ei in range(E):
                a = int(u[ei])
                bnode = int(v[ei])
                in_reg = (region[a] == 1) or (region[bnode] == 1)
        ...
        for i in range(p.N):
            ...
            drive_term = 0.0
            if in_reg:
                # same v2 rule: only positive half-cycle helps
...
def phase_stats(data: dict) -> dict:
    """
    Compute phase-binned pM_in(phi), pM_out(phi), delta(phi).
    """
    B = int(data["phi_bins"])
    eps = 1e-12
    ...
    with open(path, "w", newline="", encoding="utf-8") as f:
        ...
        for i in range(len(ph["phi_centers"])):
            ...
            if (ci < min_caps) or (co < min_caps):
                # still write row, but mark as NaN for p/delta
...
def plot_outputs(run_dir: str, data: dict, series: dict, ph: dict, p: Params) -> None:
    ...
    # hide bins with insufficient counts
    ...
    plt.legend()
    plt.savefig(os.path.join(plots, "phase_response.png"), dpi=180, bbox_inches="tight")
    plt.close()
...
def main():
    ...
    ap.add_argument("--min_captures_phase", type=int, default=200, help="hide phase bins with fewer than this many captures in/out")

    # output
    ap.add_argument("--out_dir", type=str, default="hsf_out")
    ap.add_argument("--run_name", type=str, default="metastable_toy_v3")
    ...
    tag = (
        ...
        f"_bins{p.phase_bins}_minphi{p.min_captures_phase}"
    ...
    # save artifacts
    np.savez_compressed(os.path.join(run_dir, "results.npz"), **data, **series, **ph)
    write_csv(os.path.join(run_dir, "log.csv"), data, series)
    write_phase_csv(os.path.join(run_dir, "phase_bins.csv"), ph, p.min_captures_phase)
    plot_outputs(run_dir, data, series, ph, p)

    print("Run output:", run_dir)
    print("Clean plot:", os.path.join(run_dir, "plots", "branch_clean.png"))
    print("Phase plot:", os.path.join(run_dir, "plots", "phase_response.png"))
    print("Phase CSV :", os.path.join(run_dir, "phase_bins.csv"))
