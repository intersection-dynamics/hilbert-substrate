# filename: hsf_mesoscape_entanglement_fast_ecology.py
...
from hsf_mesoscale_physics_core import PhysicsConfig, GM_MATRICES, canonical_edge, to_numpy
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description=(
            "HSF mesoscale entanglement fast ecology sandbox. "
            "Preserves the full ecology (birth, lower_support, edge_up, edge_down) "
            "while speeding evaluation through baseline caching, cheap candidate proxies, "
            "and reduced exact finalist scoring."
        )
    )
    ...
    # Speed / evaluation controls
    p.add_argument("--birth-finalists", type=int, default=4)
    p.add_argument("--lower-finalists", type=int, default=2)
    p.add_argument("--edge-up-finalists", type=int, default=3)
    p.add_argument("--edge-down-finalists", type=int, default=3)

    p.add_argument("--json-out", type=str, default="hsf_mesoscape_entanglement_fast_ecology.json")
    return p.parse_args()
    ...
    for row in parent_rows[: int(args.max_parent_pairs)]:
        ...
        for child in range(len(sigma)):
            if child in (a, b):
                continue
            attach = float(
                interface_commitment.get(canonical_edge(a, child), 0.0)
                + interface_commitment.get(canonical_edge(b, child), 0.0)
            )
...
def batched_birth_proxy(
    ...
    if not candidates:
        return []
    child = xp.asarray([c["child"] for c in candidates], dtype=xp.int32)
    a = xp.asarray([c["parents"][0] for c in candidates], dtype=xp.int32)
    b = xp.asarray([c["parents"][1] for c in candidates], dtype=xp.int32)
    ent_source = xp.asarray([c["ent_source"] for c in candidates], dtype=xp.float64)

    sigma_xp = xp.asarray(sigma, dtype=xp.float64)
    child_sigma = sigma_xp[child]

    W = xp.zeros((len(sigma), len(sigma)), dtype=xp.float64)
    for (i, j), w in interface_commitment.items():
        W[i, j] = float(w)
        W[j, i] = float(w)
    attach = W[a, child] + W[b, child]

    vacancy = 1.0 - xp.clip(child_sigma, 0.0, 1.0)
    novelty = 1.0 - 0.5 * xp.clip(attach, 0.0, 1.0)
    proxy = ent_source * vacancy * novelty
    proxy_np = to_numpy(proxy)

    out: List[Dict[str, Any]] = []
    for idx, cand in enumerate(candidates):
        row = dict(cand)
        row["birth_proxy"] = float(proxy_np[idx])
        out.append(row)
    out.sort(key=lambda r: r["birth_proxy"], reverse=True)
    return out


def rank_lower_candidates(
    sigma: np.ndarray,
    active_nodes: Set[int],
    parent_rows: List[Dict[str, Any]],
) -> List[Tuple[float, int]]:
    sustain = {i: 0.0 for i in active_nodes}
    for row in parent_rows:
        a, b = int(row["parents"][0]), int(row["parents"][1])
        src = float(row["source"])
        if a in sustain:
            sustain[a] = max(sustain[a], src)
        if b in sustain:
            sustain[b] = max(sustain[b], src)
    rows: List[Tuple[float, int]] = []
    for i in active_nodes:
        proxy = float((1.0 - sustain.get(i, 0.0)) * np.clip(sigma[i], 0.0, 1.0))
        rows.append((proxy, int(i)))
    rows.sort(reverse=True)
    return rows


def rank_edge_up_candidates(
    interface_commitment: Dict[Edge, float],
...
def run_sim(pcfg: PhysicsConfig, scfg: bk.ScoreConfig, args: argparse.Namespace):
    ...
    for step in range(1, args.total_steps + 1):
        ...
        # Baseline evolved exactly once per step.
        ...
        psi_b, active_nodes_b, _, active_edges_b, _, _, _ = baseline_state

        # Entanglement computed exactly once per step.
        ...
        ranked_births = batched_birth_proxy(raw_births, sigma, interface_commitment, xp)
        birth_rows = ranked_births[: int(args.birth_finalists)]

        lower_rows = rank_lower_candidates(sigma, active_nodes_b, parent_rows)[: int(args.lower_finalists)]
        edge_up_rows = rank_edge_up_candidates(interface_commitment, birth_rows)[: int(args.edge_up_finalists)]
        edge_down_rows = rank_edge_down_candidates(interface_commitment, active_edges_b, parent_rows)[: int(args.edge_down_finalists)]
        ...
        if int(args.progress_every) > 0 and step % int(args.progress_every) == 0:
            print(
                ...
                f"lower_fin={len(lower_rows)} up_fin={len(edge_up_rows)} down_fin={len(edge_down_rows)} "
                f"cand={len(diags)}"
            )

    return {
        "script": "hsf_mesoscape_entanglement_fast_ecology.py",
        ...
        "run_config": {
            ...
            "principle": "preserve the full ecology while evaluating it faster through cached baseline evolution, cached pair entanglement, cheap per-class proxies, and exact scoring only on reduced finalist sets",
        },
