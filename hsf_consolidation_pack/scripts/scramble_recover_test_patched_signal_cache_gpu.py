...
def objective_signal_entropy(H: np.ndarray, N: int, eps: float = 1e-12) -> float:
    ...
    Interpret S_ij = ||Tr_rest(H)||_F on (i,j) as a crude upper bound on instantaneous
    influence between sites i and j. For each i form:
        p_j ∝ S_ij   (j != i)
        H_i = -Σ p_j log p_j
    Return mean_i( H_i / log(N-1) ) in [0,1]. Lower = influence concentrated to few partners.
    """
    if N <= 2:
        return 0.0
    S = pair_strengths_matrix(H, N)
...
def isospectral_flow_step(H: np.ndarray, N: int, dt: float, p: int, max_weight: Optional[int], backend: str = "numpy") -> np.ndarray:
    """One isospectral flow step.

    Gradient (locality_gradient) is computed on CPU (NumPy). We optionally accelerate the
    expensive matrix exponential + conjugation on GPU using CuPy (FLOW only).

    backend:
      - "numpy": use SciPy expm on CPU
      - "cupy" : use cupyx.scipy.linalg.expm on GPU for U = expm(dt*G) and compute UHU† on GPU
    """
    ...
    if backend == "cupy":
        try:
            ...
            raise RuntimeError(
                "CuPy backend requested but CuPy is not available. Install with: pip install cupy-cuda12x (or cupy-cuda11x)"
            ) from e
...
def run_one(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    backend = str(payload.get("backend", "numpy")).lower()
    ...
    if recover in ("flow", "both") and flow_steps > 0:
        ...
        for _ in range(flow_steps):
            H = isospectral_flow_step(H, N, dt=dt, p=p, max_weight=max_weight_opt, backend=backend)
    ...
    # Signal entropy diagnostics (geometry-blind)
    ...
    # Soft success flag (dashboard; not a hard claim)
...
def summarize_sweep(results: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    sparse_reds = [r.get("sparse_reduction", float("nan")) for r in ok]
    sparse_reds = [x for x in sparse_reds if isinstance(x, (int, float)) and np.isfinite(x)]

    signal_reds = [r.get("signal_entropy_reduction", float("nan")) for r in ok]
    signal_reds = [x for x in signal_reds if isinstance(x, (int, float)) and np.isfinite(x)]
    ...
    s_stats = _stats(sparse_reds)
    g_stats = _stats(signal_reds)
    ...
    return {
        ...
        "median_sparse_reduction": s_stats["median"],
        "mean_sparse_reduction": s_stats["mean"],
        "min_sparse_reduction": s_stats["min"],
        "max_sparse_reduction": s_stats["max"],
        "median_signal_entropy_reduction": g_stats["median"],
        "mean_signal_entropy_reduction": g_stats["mean"],
        "min_signal_entropy_reduction": g_stats["min"],
        "max_signal_entropy_reduction": g_stats["max"],
    }
...
def main() -> None:
    ...
    parser.add_argument("--backend", choices=["numpy", "cupy"], default="numpy", help="Flow backend (FLOW only). cupy uses GPU for expm+matmuls.")
    ...
    if args.seed_count > 1:
        ...
        # If using GPU backend for FLOW, don't run multiple seeds concurrently on one GPU.
        if str(args.backend).lower() == "cupy" and jobs > 1:
            print("[warn] --backend cupy selected: forcing --jobs 1 to avoid GPU contention.")
            jobs = 1
